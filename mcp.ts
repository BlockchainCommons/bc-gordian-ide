...
async function listPackages(ctx: ToolContext): Promise<string> {
  ...
  const lines = data.packages.map(
    (p) =>
      `- **${p.label === "tool" ? p.name : `@blockchaincommons/${p.name}`}** (${p.symbolCount} symbols) — ${p.description || "no description"}\n  ${p.url}`,
  );
...
async function searchSymbols(ctx: ToolContext, query: string, pkg?: string): Promise<string> {
  ...
  if (matches.length === 0) {
    return `No symbols matched "${query}"${pkg ? ` in @blockchaincommons/${pkg}` : ""}.`;
  }
  const limited = matches.slice(0, 100);
  const lines = limited.map(
    (m) =>
      `- \`${m.kind} ${m.name}\` — @blockchaincommons/${m.package}\n  ${m.summary || "(no summary)"}\n  ${m.url}`,
  );
  const suffix =
    matches.length > limited.length
      ? `\n\n_${matches.length - limited.length} more results truncated._`
      : "";
  return `# ${matches.length} match${matches.length === 1 ? "" : "es"} for "${query}"${pkg ? ` in @blockchaincommons/${pkg}` : ""}\n\n${lines.join("\n")}${suffix}`;
}
...
async function getSymbol(ctx: ToolContext, pkg: string, name: string): Promise<string> {
  ...
  if (!sym) return `Symbol \`${name}\` not found in @blockchaincommons/${pkg}.`;
  const out: string[] = [];
  out.push(`# ${sym.kind} \`${sym.name}\` — @blockchaincommons/${pkg}`);
  out.push("");
  if (sym.summary) {
...
async function getGuide(ctx: ToolContext, pkg: string): Promise<string> {
  ...
  if (!md) return `No guide available for @blockchaincommons/${pkg}.`;
  return md;
}
...
async function findExamples(ctx: ToolContext, query: string): Promise<string> {
  ...
  for (const [pkg, syms] of byPkg) {
    ...
    for (const s of syms) {
      ...
      sections.push(
        `## @blockchaincommons/${pkg} · \`${full.kind} ${full.name}\`\n\n${block ? "```ts\n" + block + "\n```" : full.summary}\n\n${full.url}`,
      );
      total++;
    }
  }
