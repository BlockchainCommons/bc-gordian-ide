import { Service, type Privilege, type Key } from "@blockchaincommons/xid";
import "@blockchaincommons/envelope/all";
import type { Envelope } from "@blockchaincommons/envelope/all";
import { Reference, type PublicKeys } from "@blockchaincommons/components";
...
export function useXidTutorial() {
  ...
  const keyList = computed(() => {
    void docVersion.value;
    return activeDoc.value ? [...activeDoc.value.keys] : [];
  });
  const resolutionMethodList = computed(() => {
    void docVersion.value;
    // `XIDDocument.resolutionMethods` is a set of `URI`s (mirrors Rust). The
    // composable surfaces a string view for the existing template bindings.
  ...
  const attachmentList = computed(() => {
    ...
    for (const envelope of doc.attachments) {
      ...
      try {
        const text = envelope.attachmentPayload().asText();
  ...
  const serviceList = computed(() => {
    void docVersion.value;
    return activeDoc.value ? [...activeDoc.value.services] : [];
  });
  const edgeList = computed(() => {
    ...
    for (const env of doc.edges()) {
      out.push({ digestHex: env.digest().toHex(), envelope: env });
    }
    return out;
  });
  ...
  function buildOperationalEnvelope(alsoRemoveNicknames: string[] = []): Envelope | null {
    ...
    try {
      ...
      const copy = loadXidFromUr(persist.toUR().toString(), slot.password || undefined);
      copy.removeInceptionKey();
      for (const name of alsoRemoveNicknames) removeKeyByNickname(copy, name);
      return copy.toEnvelope({ privateKeys: "elide", generator: "elide", sign: "none" });
    } catch (e) {
  ...
  function advanceProvenance() {
    ...
    try {
      ...
      doc.nextProvenanceMarkWithEmbeddedGenerator({ password: pw, date: new Date() });
      invalidatePrivateEnvelope(activeIdentity.value);
      bumpDoc();
      saveState();
    } catch (e) {
  ...
  function addService(uri: string, name: string, capability: string, keyRefHex?: string) {
    ...
    try {
      ...
      if (keyRefHex) {
        service.addKeyReference(Reference.fromHex(keyRefHex));
  ...
  function getPrivateEnvelope(name?: IdentityName): Envelope | null {
    ...
    try {
      const env = slot.document.toEnvelope({
        privateKeys: privateKeyOptions(slot.password),
        generator: generatorOptions(slot.password),
        sign: "inception",
      });
  ...
  function getPersistEnvelope(name?: IdentityName): Envelope | null {
    ...
    if (!slot.document) return null;
    try {
      return slot.document.toEnvelope({
        privateKeys: "include",
        generator: "include",
        sign: "inception",
      });
  ...
  function verifySignature(): boolean {
    ...
    if (!env) return false;
    return verifyInceptionSignature(env.toUR().toString());
  }
