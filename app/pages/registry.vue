...
const activeTab = ref<RegistryTab>("known-values");
...
const categoryColors: Record<string, string> = {
  ...
  Community: "bg-stone-100 text-stone-700 dark:bg-stone-900/30 dark:text-stone-400",
  RDF: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400",
  RDFS: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400",
  OWL2: "bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400",
  DCE: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400",
  DCT: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400",
  FOAF: "bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400",
  SKOS: "bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400",
  Solid: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400",
  VC: "bg-fuchsia-100 text-fuchsia-700 dark:bg-fuchsia-900/30 dark:text-fuchsia-400",
  GS1: "bg-lime-100 text-lime-700 dark:bg-lime-900/30 dark:text-lime-400",
  Schema: "bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-400",
};
  ...
  <UDashboardPanel id="registry">
    ...
    <template #body>
      <div class="flex flex-col h-full overflow-hidden bg-white dark:bg-gray-900">
        ...
        <div class="flex-1 overflow-auto">
          ...
          <table v-if="activeTab === 'tags'" class="w-full">
            <thead class="sticky top-0 bg-gray-50 dark:bg-gray-800/80 backdrop-blur z-10">
              <tr>
                <th
                  class="text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-4 py-3 w-32"
                >
                  Tag
                </th>
                <th
                  class="text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-4 py-3"
                >
                  Name
                </th>
                <th
                  class="text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-4 py-3 w-48"
                >
                  Category
                </th>
              </tr>
              ...
              >
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      v-if="tag.deprecated"
                      class="px-1.5 py-0.5 text-[10px] font-medium rounded bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400"
                      >DEPRECATED</span
                    >
                  </div>
                </td>
          ...
          <table v-else-if="activeTab === 'known-values'" class="w-full">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800/50">
              ...
              >
                <td class="px-4 py-3">
                  <code
                    class="text-sm font-mono font-semibold text-emerald-600 dark:text-emerald-400"
                  >
                    {{ kv.value }}
                  </code>
                </td>
          ...
          <div v-else-if="activeTab === 'iana'">
            ...
            <div v-if="ianaStats.loading" class="flex items-center justify-center h-64">
              <div class="text-center">
                <UIcon
                  ...
                  class="w-8 h-8 text-gray-400 dark:text-gray-500 animate-spin mb-3"
                />
            ...
            <div v-else-if="ianaStats.error" class="flex items-center justify-center h-64">
              <div class="text-center">
                ...
                <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-200 mb-1">
                  Failed to load IANA registry
                </h3>
            ...
            <table v-else class="w-full">
              <thead class="sticky top-0 bg-gray-50 dark:bg-gray-800/80 backdrop-blur z-10">
                <tr>
                  ...
                  <th
                    class="text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-4 py-3 w-40"
                  >
                    Data Item
                  </th>
                  <th
          ...
          <div
            v-if="
              ...
              (activeTab === 'known-values' && filteredKnownValues.length === 0) ||
              (activeTab === 'iana' &&
                !ianaStats.loading &&
                !ianaStats.error &&
                filteredIanaTags.length === 0)
            "
        ...
        <!-- Status Bar -->
        <div
          class="shrink-0 flex items-center justify-between px-4 py-1.5 border-t border-gray-200 dark:border-gray-800/50 bg-gray-50 dark:bg-gray-900/50 text-xs text-gray-500 dark:text-gray-400"
        >
