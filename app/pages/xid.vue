  ...
  meta: [
    {
      name: "description",
      content:
        "Learn to create, verify, and use eXtensible IDentifiers (XIDs) with this interactive tutorial",
    },
  ],
});
...
onMounted(() => {
  ...
  if (Number.isFinite(storedPane) && storedPane >= 20 && storedPane <= 80)
    mainPaneSize.value = storedPane;
  const storedSidebar = Number(localStorage.getItem(SIDEBAR_STORAGE_KEY));
  if (
    Number.isFinite(storedSidebar) &&
    storedSidebar >= SIDEBAR_MIN &&
    storedSidebar <= SIDEBAR_MAX
  ) {
...
watch(mainPaneSize, (v) => {
  try {
    localStorage.setItem(PANE_STORAGE_KEY, String(v));
  } catch {
    /* */
  }
});
...
function resetMainPaneSize() {
  mainPaneSize.value = 50;
}
...
function resetSidebarSize() {
  sidebarSize.value = SIDEBAR_DEFAULT;
}
    ...
    <template #body>
      <ClientOnly>
        ...
        <div class="h-full flex flex-col overflow-hidden">
          ...
          <div
            class="lg:hidden border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/30 shrink-0"
          >
            <button
              type="button"
            ...
            >
              <UIcon
                :name="mobileNavOpen ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'"
              ...
              <span class="text-gray-600 dark:text-gray-400 truncate">{{
                identities[activeIdentity].displayName
              }}</span>
            </button>
            <div
              v-if="mobileNavOpen"
              class="px-4 pb-4 pt-1 space-y-4 max-h-[60vh] overflow-y-auto border-t border-gray-200/70 dark:border-gray-800/70"
            >
              <div>
                ...
                <div class="space-y-1">
                  ...
                  >
                    ...
                    <span
                      ...
                      :class="!slot.document && 'text-gray-400'"
                    >
                      {{ slot.displayName }}
                    </span>
                    <UBadge v-if="slot.document" size="xs" color="success" variant="subtle"
                      >ready</UBadge
                    >
                  </button>
                </div>
                ...
                <div class="space-y-0.5">
                  <button
                    ...
                    :class="[
                      ...
                      globalIndex(s.id) === currentSection
                  ...
                  >
                    <UIcon
                      :name="
                        sectionsCompleted[globalIndex(s.id)]
      ...
      -->
          <div
            ref="layoutRef"
            class="flex-1 min-h-0 flex max-lg:portrait:flex-col lg:flex-row gap-0 overflow-hidden"
          >
            <!-- Desktop sidebar -->
            <aside
              :style="sidebarStyle"
            ...
            <div
              class="hidden lg:flex group shrink-0 items-center justify-center select-none touch-none transition-colors cursor-col-resize w-1.5 h-full bg-gray-200 dark:bg-gray-800 hover:bg-primary-300 dark:hover:bg-primary-700"
              role="separator"
            ...
            >
              <span
                class="block w-0.5 h-8 bg-gray-400 dark:bg-gray-600 group-hover:bg-primary-500 rounded-full transition-colors"
              />
            </div>
            ...
            >
              ...
              <div
                v-show="hasOutput"
                class="group shrink-0 flex items-center justify-center select-none touch-none transition-colors cursor-col-resize bg-gray-200 dark:bg-gray-800 hover:bg-primary-300 dark:hover:bg-primary-700 max-lg:portrait:cursor-row-resize max-lg:portrait:w-full max-lg:portrait:h-1.5 lg:w-1.5 lg:h-full max-lg:landscape:w-1.5 max-lg:landscape:h-full"
                role="separator"
                aria-orientation="vertical"
              ...
              >
                <span
                  class="block bg-gray-400 dark:bg-gray-600 group-hover:bg-primary-500 rounded-full transition-colors max-lg:portrait:h-0.5 max-lg:portrait:w-8 lg:w-0.5 lg:h-8 max-lg:landscape:w-0.5 max-lg:landscape:h-8"
                />
              </div>
              ...
              >
                <div
                  class="hidden lg:block px-4 py-3 border-b border-gray-200 dark:border-gray-800/50 bg-gray-50 dark:bg-gray-900/50 shrink-0"
                >
