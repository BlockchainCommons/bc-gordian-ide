...
import {
  ...
  Request as EnvRequest,
  Response as EnvResponse,
  Event as EnvEvent,
  ObscureType,
} from "@blockchaincommons/envelope/all";
import type { TreeFormatOptions } from "@blockchaincommons/envelope/all";
import {
  SignatureScheme,
  createKeypair,
  SigningPrivateKey,
...
import type { SigningPublicKey } from "@blockchaincommons/components";
...
useHead({
  ...
  meta: [
    {
      name: "description",
      content: "Build Gordian Envelopes layer by layer with a visual tree builder",
    },
  ],
});
...
type NodeType =
  "subject" | "assertion" | "wrapped" | "signed" | "encrypted" | "compressed" | "elided" | "salted";
...
const knownValuesList = [
  ...
  {
    label: "dereferenceVia",
    value: "dereferenceVia",
    kv: DEREFERENCE_VIA,
...
const sskrGroups = ref<{ memberThreshold: number; memberCount: number }[]>([
...
function createRecipient(name: string, scheme: RecipientScheme = "X25519"): RecipientKey {
  const privateKey = PrivateKeyBase.random();
  // Get hex representation of public key for display
  const publicKeyHex = privateKey.schnorrPublicKeys().reference().shortReference("hex");
  return {
    id: generateId(),
    name,
    privateKey,
...
function createSigningKey(name: string, algorithm: SigningAlgorithm = "Ed25519"): SigningKey {
  // ML-DSA private keys cannot derive their public key, so keep the one the keypair factory returns
  let privateKey: SigningPrivateKey;
  let publicKey: SigningPublicKey;
  switch (algorithm) {
    case "Schnorr":
      privateKey = SigningPrivateKey.random({ scheme: SignatureScheme.Schnorr });
      publicKey = privateKey.publicKey();
      break;
    case "ECDSA":
      privateKey = SigningPrivateKey.random({ scheme: SignatureScheme.Ecdsa });
      publicKey = privateKey.publicKey();
      break;
    case "ML-DSA-44":
      [privateKey, publicKey] = createKeypair(SignatureScheme.MLDSA44);
      break;
    case "ML-DSA-65":
      [privateKey, publicKey] = createKeypair(SignatureScheme.MLDSA65);
      break;
    case "ML-DSA-87":
      [privateKey, publicKey] = createKeypair(SignatureScheme.MLDSA87);
      break;
    default:
      privateKey = SigningPrivateKey.random({ scheme: SignatureScheme.Ed25519 });
      publicKey = privateKey.publicKey();
  }
...
onMounted(() => {
  demoSigningKey.value = SigningPrivateKey.random({ scheme: SignatureScheme.Ed25519 });
  demoEncryptionKey.value = SymmetricKey.random();
  // Initialize with demo recipients
  recipients.value = [createRecipient("Alice"), createRecipient("Bob")];
...
// Parse a `ur:envelope/...` string into an Envelope
function envelopeFromUR(urString: string): Envelope {
  return decodeURWith(UR.parse(urString), Envelope.codec);
}
    ...
    switch (node.type) {
      case "subject": {
        ...
        for (const assertion of node.assertions ?? []) {
          if (assertion.type === "assertion" && assertion.object) {
            ...
            if (objEnv) {
              const predicateValue = parsePredicateValue(
                assertion.predicateType,
                assertion.predicate!,
                assertion.predicateKnownValue,
              );
              if (assertion.salted) {
                env = env.addAssertion(predicateValue, objEnv, { salt: true });
              } else {
      ...
      case "signed":
        if (node.child) {
          ...
          const signerIds =
            node.signerIds && node.signerIds.length > 0
              ? node.signerIds
          ...
          for (const signerId of signerIds) {
            ...
            if (node.signatureNote) {
              const metadata = SignatureMetadata.from([["note", node.signatureNote]]);
              signedEnv = signedEnv.addSignature(signerKey, { metadata });
            } else {
        ...
        if (node.child) {
          ...
          if (
            ...
            if (selectedRecipients.length > 0) {
              const recipientPublicKeys = selectedRecipients.map((r) =>
                r.privateKey.schnorrPublicKeys(),
              );
        ...
        if (node.child) {
          ...
          if (childEnv) {
            switch (node.saltMode) {
              case "fixed":
                return childEnv.addSalt({ length: node.saltLength ?? 16 });
              case "range":
                return childEnv.addSalt({
                  range: { min: node.saltMin ?? 8, max: node.saltMax ?? 32 },
                });
              default:
                return childEnv.addSalt();
            }
...
function parseValue(type: ValueType | undefined, value: unknown): EnvelopeValue {
  switch (type) {
    ...
    case "date":
      return CborDate.fromDate(new Date(String(value)));
    case "uuid":
      return UUID.fromString(String(value));
    case "arid":
      return ARID.fromHex(String(value).replace(/^0x/, ""));
    case "uri":
      return URI.from(String(value));
    case "known": {
...
function updateOutput() {
  ...
  try {
    ...
    if (envelope) {
      ...
      if (treeDigestDisplay.value !== "short") treeOpts.digestDisplay = treeDigestDisplay.value;
      ...
      try {
        mermaidOutput.value = envelope.mermaidFormat({
...
function openQRModal() {
  if (!hexOutput.value) {
    toast.add({
      title: "Create an envelope first",
      color: "warning",
...
function wizardComplete() {
  ...
  toast.add({
    title: "Envelope created with wizard",
    color: "success",
...
function addAssertion() {
  ...
  const predicateDisplay =
    assertionPredicateType.value === "known"
  ...
  const assertion: EnvelopeNode = {
    ...
    predicateKnownValue:
      assertionPredicateType.value === "known" ? assertionPredicateKnownValue.value : undefined,
...
function verifySignature(nodeId: string) {
  ...
  try {
    ...
    if (!envelope) {
      verifyResult.value = {
        verified: false,
        message: "Failed to build envelope for verification",
      };
      showVerifyModal.value = true;
      return;
    }
    ...
    for (const key of signingKeys.value) {
      try {
        if (envelope.hasSignatureFrom(key.publicKey)) {
    ...
    if (allVerified.length > 0) {
      ...
      verifyResult.value = {
        verified: false,
        message:
          "No matching signature found. The envelope may have been signed with an unknown key.",
      };
    }
  } catch (e) {
    verifyResult.value = {
...
function applySelectiveElision() {
  ...
  showElideSelectModal.value = false;
  toast.add({
    title: "Selective elision applied",
    color: "success",
...
function applyElisionToTree(node: EnvelopeNode): EnvelopeNode | null {
  ...
  if (node.assertions) {
    node.assertions = node.assertions.map((assertion) => {
      ...
      const assertionShouldElide =
        elideMode.value === "reveal" ? !assertionShouldReveal : assertionShouldReveal;
      ...
      if (assertion.type === "elided" && !assertionShouldElide && assertion.child) {
        ...
        if (restoredAssertion.object) {
          restoredAssertion.object =
            applyElisionToTree(restoredAssertion.object) ?? restoredAssertion.object;
        }
...
function getSelectableNodes(
  node: EnvelopeNode,
  depth: number = 0,
): Array<{ id: string; label: string; depth: number; type: NodeType; isElided: boolean }> {
  const nodes: Array<{
    id: string;
    label: string;
    depth: number;
...
function getNodeDisplayLabel(node: EnvelopeNode): string {
  ...
  if (node.type === "subject") {
    ...
    if (node.valueType === "string")
      return `"${String(node.value).substring(0, 20)}${String(node.value).length > 20 ? "..." : ""}"`;
...
function sealNode() {
  ...
  if (!sealSignerId.value || !sealRecipientId.value) {
    toast.add({
      title: "Select both a signer and recipient",
      color: "warning",
      icon: "i-heroicons-exclamation-triangle",
    });
    return;
  ...
  const recipientName =
    recipients.value.find((r) => r.id === sealRecipientId.value)?.name || "Unknown";
  toast.add({
...
function performSskrSplit() {
  ...
  try {
    ...
    // Create SSKR spec
    const groupSpecs = sskrGroups.value.map((g) =>
      GroupSpec.from({ memberThreshold: g.memberThreshold, memberCount: g.memberCount }),
    );
    const spec = Spec.from({ groupThreshold: sskrGroupThreshold.value, groups: groupSpecs });
    ...
    sskrShares.value = shareEnvelopes.map((group) =>
      group.map((shareEnv) => shareEnv.toUR().toString()),
    );
...
function performSskrJoin() {
  ...
  try {
    const shareLines = sskrJoinInput.value
      .trim()
      .split("\n")
      .map((l) => l.trim())
      .filter((l) => l.length > 0);
    ...
    const shareEnvelopes = shareLines.map((line) => envelopeFromUR(line));
...
function formatDigestValue(d: Digest): string {
  switch (digestFormat.value) {
    case "short":
      return d.toHex().substring(0, 7);
    case "ur":
      return d.toUR().toString();
    default:
      return d.toHex();
  }
}
...
function updateDigestDisplay() {
  ...
  try {
    ...
    allDigests.value = Array.from(digestSet).map((d) => formatDigestValue(d));
  } catch {
    allDigests.value = [];
  }
}
...
function findObscuredNodes() {
  ...
  try {
    ...
    for (const d of elidedDigests) {
      results.push({ digest: d.toHex().substring(0, 12), type: "elided" });
    }
    for (const d of encryptedDigests) {
      results.push({ digest: d.toHex().substring(0, 12), type: "encrypted" });
    }
    for (const d of compressedDigests) {
      results.push({ digest: d.toHex().substring(0, 12), type: "compressed" });
    }
...
function performUnelide() {
  ...
  try {
    ...
    if (input.startsWith("ur:")) {
      restoreEnvelope = envelopeFromUR(input);
    } else {
      // Try hex first
      const bytes = new Uint8Array(input.match(/.{1,2}/g)!.map((b) => parseInt(b, 16)));
      restoreEnvelope = Envelope.fromBytes(bytes);
    }
    ...
    showUnelideModal.value = false;
    toast.add({
      title: "Envelope unelided",
      description: "Elided node restored successfully.",
      color: "success",
...
function performDecrypt() {
  ...
  try {
    ...
    toast.add({
      ...
      description: "Encrypted content revealed.",
      color: "success",
...
function decompressEnvelope() {
  ...
  try {
    ...
    const decompressed = envelope.isCompressed()
      ? envelope.decompress()
      : envelope.decompressSubject();
    root.value = envelopeToNode(decompressed);
    toast.add({
      title: "Envelope decompressed",
      description: "Compressed content expanded.",
      color: "success",
...
function performUnseal() {
  ...
  try {
    ...
    const unsealed = envelope.unseal(signer.publicKey, recipient.privateKey);
    root.value = envelopeToNode(unsealed);
    showUnsealModal.value = false;
    toast.add({
      title: "Envelope unsealed",
      description: "Decrypted and signature verified.",
      color: "success",
      icon: "i-heroicons-shield-check",
    });
  } catch (e) {
...
function performWalkReplace() {
  ...
  try {
    ...
    const targetDigest = Digest.from(digestBytes);

    // Parse the replacement envelope
    let replacement: Envelope;
    const input = walkReplaceEnvelopeInput.value.trim();
    if (input.startsWith("ur:")) {
      replacement = envelopeFromUR(input);
    } else {
      const bytes = new Uint8Array(input.match(/.{1,2}/g)!.map((b) => parseInt(b, 16)));
      replacement = Envelope.fromBytes(bytes);
    }
    ...
    toast.add({
      ...
      description: "Matching nodes replaced successfully.",
      color: "success",
...
function generateSeed(size: number = 16) {
  const seed = Seed.random({ length: size });
  generatedItems.value = [
    { type: `Seed (${size}B)`, value: seed.toHex() },
    ...generatedItems.value,
  ];
...
function generateArid() {
  ...
  generatedItems.value = [{ type: "ARID", value: arid.toHex() }, ...generatedItems.value];
  showGeneratePanel.value = true;
}

function generateNonce() {
  const nonce = Nonce.random();
  generatedItems.value = [{ type: "Nonce", value: nonce.toHex() }, ...generatedItems.value];
  showGeneratePanel.value = true;
}
...
function generateDigestFromInput(input: string) {
  ...
  generatedItems.value = [{ type: "Digest", value: digest.toHex() }, ...generatedItems.value];
  showGeneratePanel.value = true;
}
...
function buildExpressionEnvelope(): Envelope | null {
  try {
    switch (expressionMode.value) {
      case "expression": {
        const func =
          exprFunctionType.value === "known"
            ? EnvFunction.named(exprKnownFunction.value)
            : EnvFunction.named(exprFunctionName.value);
      ...
      case "request": {
        ...
        const func =
          exprFunctionType.value === "known"
            ? EnvFunction.named(exprKnownFunction.value)
            : EnvFunction.named(exprFunctionName.value);
        let request = EnvRequest.from(func, id);
        for (const p of exprParams.value) {
          if (p.name && p.value) {
            const val = parseValue(p.valueType, p.value);
      ...
      case "event": {
        const id = ARID.random();
        let event = EnvEvent.from(eventContent.value || "event", id);
        if (eventNote.value) event = event.withNote(eventNote.value);
        if (eventDate.value) event = event.withDate(new Date());
...
function _queryAssertions() {
  ...
  try {
    ...
    const predValue = parsePredicateValue(
      queryPredicateType.value,
      queryPredicate.value,
      queryPredicateKnownValue.value,
    );
    ...
    queryResults.value = matches.map((a) => {
      ...
      return {
        predicate: predStr,
        object: objStr,
        digest: a.digest().toHex().substring(0, 12),
      };
    });
...
function _getNodeLabel(type: NodeType): string {
  switch (type) {
    ...
    case "elided":
      return "Elided (digest only)";
    case "salted":
      return "Salted (decorrelated)";
    default:
      return "";
  }
}
  ...
  switch (exportFormat.value) {
    ...
    case "base64": {
      ...
      const bytes = new Uint8Array(
        hexOutput.value.match(/.{2}/g)?.map((b) => parseInt(b, 16)) || [],
      );
    ...
    case "ur": {
      ...
      const urBytes = new Uint8Array(
        hexOutput.value.match(/.{2}/g)?.map((b) => parseInt(b, 16)) || [],
      );
      const base64url = btoa(String.fromCharCode(...urBytes))
        .replace(/\+/g, "-")
        .replace(/\//g, "_")
        .replace(/=/g, "");
...
async function copyExport() {
  ...
  toast.add({
    title: `${exportFormat.value.toUpperCase()} copied`,
    color: "success",
    icon: "i-heroicons-clipboard-document-check",
  });
}
...
function envelopeToNode(envelope: Envelope): EnvelopeNode {
  ...
  switch (envCase.type) {
    case "leaf": {
      ...
      try {
        const str = envelope.expectString();
        valueType = "string";
        value = str;
      } catch {
        try {
          const num = envelope.expectNumber();
          valueType = "number";
          value = num;
        } catch {
          try {
            const bool = envelope.expectBoolean();
            valueType = "bool";
            value = bool;
          } catch {
            try {
              ...
              try {
                const bytes = envelope.expectBytes();
                valueType = "bytes";
                ...
                valueType = "string";
                value = envelope.summary({ maxLength: 80 });
              }
            }
          }
        }
    ...
    case "assertion": {
      ...
      try {
        const kv = pred.expectKnownValue();
        predicateStr = kv.name;
        predicateType = "known";
        predicateKnownValue = kv.name;
      } catch {
        try {
          predicateStr = pred.expectString();
        } catch {
          predicateStr = pred.summary({ maxLength: 40 });
        }
      }
...
function importEnvelope() {
  ...
  try {
    ...
    switch (importFormat.value) {
      ...
      case "ur": {
        envelope = envelopeFromUR(importInput.value.trim());
        break;
      }
      default:
...
function addTypeToNode() {
  ...
  showTypeModal.value = false;
  toast.add({
    title: `Type "${typeValue.value}" added`,
    color: "success",
...
function addAttachmentToNode() {
  if (
    !attachmentTargetNodeId.value ||
    !root.value ||
    !attachmentPayload.value.trim() ||
    !attachmentVendor.value.trim()
  )
    return;
...
function buildTargetEnvelope(node: EnvelopeNode): Envelope | null {
  try {
    switch (node.type) {
      case "subject": {
        const value = parseValue(node.valueType, node.value);
        return Envelope.from(value);
      }
      case "assertion": {
        if (node.predicate && node.object) {
          const objectEnv = buildTargetEnvelope(node.object);
          if (objectEnv) {
            return Envelope.assertion(node.predicate, objectEnv);
          }
        }
        return null;
      }
...
function openProofModal(nodeId: string) {
  ...
  try {
    ...
    const proofBytes = proof.toCbor().toData();
    ...
    proofOutput.value =
      ...
      `Target: ${getNodeDisplay(targetNode)}\n` +
      `Root Digest: ${digestOutput.value}\n` +
      `Proof Digest: ${proof.digest().toHex()}\n` +
    ...
    <template #body>
      <div class="flex h-full">
        ...
        >
            ...
            <div v-if="!root" class="h-full flex flex-col items-center justify-center">
              <div class="text-center max-w-sm">
                ...
                <div class="flex gap-2 justify-center">
                  <UButton icon="i-heroicons-plus" color="primary" @click="openSubjectModal">
                    Add Subject
                  </UButton>
                  <UButton
                    icon="i-heroicons-document-duplicate"
          ...
          <div
            v-if="root"
            class="px-4 py-2 border-t border-gray-200 dark:border-gray-800/50 bg-gray-50 dark:bg-gray-900/50"
          >
            <div class="flex items-center gap-2 text-xs text-gray-500 flex-wrap">
              ...
              <UButton
                ...
                variant="soft"
                color="neutral"
                @click="openPasswordEncryptModal(root.id)"
              >
              ...
              <UButton
                ...
                variant="soft"
                color="neutral"
                @click="openElideSelectModal(root.id)"
              >
              ...
              <template v-if="false">
                <UButton
                  ...
                  variant="soft"
                  color="neutral"
                  @click="compressNode(root.id, true)"
                >
                ...
                <UButton
                  ...
                  variant="soft"
                  color="neutral"
                  @click="openSskrSplitModal(root.id)"
                >
              ...
              <template v-if="false">
                ...
                <UButton
                  ...
                  variant="soft"
                  color="neutral"
                  @click="openExpressionModal('expression')"
                >
                ...
                <UButton
                  ...
                  variant="soft"
                  color="neutral"
                  @click="openExpressionModal('request')"
                >
                ...
                <UButton
                  ...
                  variant="soft"
                  color="neutral"
                  @click="openExpressionModal('response')"
                >
                ...
                <UButton
                  ...
                  variant="soft"
                  color="neutral"
                  @click="openExpressionModal('event')"
                >
          ...
          <div class="flex-1 overflow-auto p-4 space-y-4">
            ...
            <div v-if="digestOutput && !error">
              <div class="flex items-center justify-between mb-2">
                ...
                >
                  ...
                  <UBadge
                    v-if="digestDepth !== 'top'"
                    color="neutral"
                    variant="soft"
                    size="xs"
                    class="ml-1"
                    >{{ allDigests.length }}</UBadge
                  >
                </h4>
                <div class="flex items-center gap-1">
                  ...
                  <UButton
                    ...
                    @click="
                      copyToClipboard(
                        digestDepth === 'top' ? digestOutput : allDigests.join('\n'),
                        'Digest',
                      )
                    "
                  />
              ...
              <div
                v-else
                class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-2 text-xs font-mono text-gray-600 dark:text-gray-400 max-h-48 overflow-y-auto space-y-0.5"
              >
              ...
              <div class="flex items-center justify-between mb-2">
                ...
                <div class="flex items-center gap-2">
                  <label class="flex items-center gap-1 text-xs text-gray-500">
                    <input
                      v-model="treeHideNodes"
                      type="checkbox"
                      class="rounded text-primary-500 w-3 h-3"
                    />
                    Hide nodes
                  </label>
                  <select
            ...
            <div v-if="showObscuredPanel && obscuredNodes.length > 0">
              <div class="flex items-center justify-between mb-2">
                ...
                >
                  Obscured Nodes
                  <UBadge color="warning" variant="soft" size="xs" class="ml-1">{{
                    obscuredNodes.length
                  }}</UBadge>
                </h4>
                <UButton
            ...
            <div v-if="showObscuredPanel && obscuredNodes.length === 0 && !error">
              <UAlert
                ...
                title="No obscured nodes found"
                description="This envelope has no elided, encrypted, or compressed elements."
              />
            </div>
            ...
            <div v-if="showGeneratePanel">
              ...
              <div class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 space-y-3">
                <div class="flex gap-2 flex-wrap">
                  <UButton size="xs" variant="soft" color="primary" @click="generateSeed(16)"
                    >Seed (16B)</UButton
                  >
                  <UButton size="xs" variant="soft" color="primary" @click="generateSeed(32)"
                    >Seed (32B)</UButton
                  >
                  <UButton size="xs" variant="soft" color="primary" @click="generateArid()"
                    >ARID</UButton
                  >
                  <UButton size="xs" variant="soft" color="primary" @click="generateNonce()"
                    >Nonce</UButton
                  >
                </div>
                <div class="flex gap-2">
                  <UInput
                    v-model="digestInputText"
                    placeholder="Text to hash..."
                    size="xs"
                    class="flex-1"
                    @keydown.enter="generateDigestFromInput(digestInputText)"
                  />
                  <UButton
                    size="xs"
                    variant="soft"
                    color="primary"
                    @click="generateDigestFromInput(digestInputText)"
                    >Digest</UButton
                  >
                </div>
            ...
            <!-- Empty State -->
            <div
              v-if="!treeOutput && !hexOutput && !error"
      ...
      <UModal
        ...
        description="Choose a type and value for the envelope subject."
      >
        <template #content>
          <div class="p-6">
              ...
              <div v-if="subjectValueType !== 'null'">
                ...
                <template v-else-if="subjectValueType === 'known'">
                  <USelectMenu
                    v-model="subjectValue"
                    :items="
                      knownValuesList.map((kv) => ({
                        label: `${kv.label} - ${kv.description}`,
                        value: kv.value,
                      }))
                    "
                    value-key="value"
                ...
                <template v-else-if="subjectValueType === 'date'">
                  <UInput v-model="subjectValue" type="datetime-local" />
                </template>
                <template v-else>
                  <UInput
                    v-model="subjectValue"
                    ...
                    :placeholder="
                      subjectValueType === 'bytes'
                        ? 'Hex string (e.g., deadbeef)'
                        : subjectValueType === 'bigint'
                          ? 'Large integer (e.g., 9007199254740993)'
                          : subjectValueType === 'uuid'
                            ? 'UUID (e.g., eb377e65-5774-410a-b9cb-510bfc73e6d9)'
                            : subjectValueType === 'arid'
                              ? 'Hex string (32 bytes)'
                              : subjectValueType === 'uri'
            ...
            <div class="flex justify-end gap-2 mt-6">
              ...
              <UButton color="primary" @click="createSubject"> Create </UButton>
            </div>
          </div>
        </template>
      </UModal>
      ...
      <!-- Assertion Modal -->
      <UModal
        v-model:open="showAssertionModal"
        title="Add Assertion"
        description="Define a predicate and object value for the assertion."
      >
        <template #content>
          <div class="p-6">
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <template v-else>
                  <UInput v-model="assertionPredicate" placeholder="e.g., name, age, email" />
                </template>
              </div>
              ...
              <div v-if="assertionValueType !== 'null'">
                ...
                <template v-else-if="assertionValueType === 'date'">
                  <UInput v-model="assertionValue" type="datetime-local" />
                </template>
                <template v-else>
                  <UInput
                    v-model="assertionValue"
                    ...
                    :placeholder="
                      assertionValueType === 'bytes'
                        ? 'Hex string (e.g., deadbeef)'
                        : assertionValueType === 'bigint'
                          ? 'Large integer'
                          : assertionValueType === 'uuid'
                            ? 'UUID (e.g., eb377e65-5774-410a-b9cb-510bfc73e6d9)'
                            : assertionValueType === 'arid'
                              ? 'Hex string (32 bytes)'
                              : assertionValueType === 'uri'
                                ? 'URI (e.g., https://example.com)'
                                : 'Enter value'
                    "
                  />
              ...
              <!-- Salted toggle -->
              <label
                class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50"
                :class="{ 'border-orange-500 bg-orange-50 dark:bg-orange-900/20': assertionSalted }"
              >
                <input
                  v-model="assertionSalted"
                  type="checkbox"
                ...
                <div class="flex-1">
                  ...
                  <div class="text-xs text-gray-500">
                    Add random salt to decorrelate the assertion digest
                  </div>
                </div>
                <UIcon
                  v-if="assertionSalted"
            ...
            <div class="flex justify-end gap-2 mt-6">
              ...
              <UButton
                color="primary"
                :disabled="
                  assertionPredicateType === 'string'
                    ? !assertionPredicate
                    : !assertionPredicateKnownValue
                "
                @click="addAssertion"
              >
                Add
      ...
      <!-- Salt Modal -->
      <UModal
        v-model:open="showSaltModal"
        title="Add Salt"
        description="Configure salt to decorrelate the envelope digest."
      >
        <template #content>
          <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Add Salt</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Salt adds random data to decorrelate the envelope's digest, preventing correlation
              attacks.
            </p>
                ...
                <div class="space-y-2">
                  ...
                  >
                    ...
                    <div class="flex-1">
                      ...
                      <div class="text-xs text-gray-500">
                        Let the library choose salt length automatically
                      </div>
                    </div>
                    <UIcon
                      v-if="saltMode === 'auto'"
      ...
      <UModal
        v-model:open="showElideSelectModal"
        title="Selective Elision"
        description="Choose which parts of the envelope to reveal or hide."
      >
        <template #content>
          <div class="p-6 max-h-[80vh] flex flex-col">
            ...
            <div class="mb-4">
              ...
              <p class="text-xs text-gray-500 mt-1">
                {{
                  elideMode === "reveal"
                    ? "Check items to keep visible, unchecked will be elided"
                    : "Check items to elide, unchecked will remain visible"
                }}
              </p>
            </div>
            ...
            <div class="mb-4">
              ...
              <p class="text-xs text-gray-500 mt-1">
                {{
                  elideAction === "elide"
                    ? "Replace with digest (irreversible without original)"
                    : elideAction === "encrypt"
                      ? "Encrypt with symmetric key (recoverable with key)"
                      : "Compress (fully recoverable)"
                }}
              </p>
            </div>

            <div
              class="flex-1 overflow-auto border border-gray-200 dark:border-gray-700 rounded-lg p-2 bg-gray-50 dark:bg-gray-900/50"
            >
              <template v-if="root">
                <div
      ...
      <!-- Recipients Modal -->
      <UModal
        v-model:open="showRecipientsModal"
        title="Multi-Recipient Encryption"
        description="Select recipients who can decrypt this envelope."
      >
        <template #content>
          <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
              ...
              <UAlert
                v-if="
                  ...
                  !(
                    encryptTargetNodeId &&
                    root &&
                    findNode(root, encryptTargetNodeId)?.type === 'encrypted'
                  )
                "
                color="warning"
                variant="soft"
      ...
      <UModal
        v-model:open="showKeyManagementModal"
        title="Key Management"
        description="Manage signing keys, encryption keys, and recipients."
      >
        <template #content>
          <div class="p-6 max-w-lg">
            ...
            <div class="mb-6">
              ...
              <div
                class="border border-gray-200 dark:border-gray-700 rounded-lg divide-y divide-gray-200 dark:divide-gray-700"
              >
                <div v-for="key in signingKeys" :key="key.id" class="flex items-center gap-3 p-3">
                  <input
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="font-medium text-sm text-gray-900 dark:text-white">
                      {{ key.name }}
                      <span
                        class="ml-1 text-xs px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400"
                        >{{ key.algorithm }}</span
                      >
                    </div>
                    <div class="text-xs font-mono text-gray-500 truncate">
            ...
            <div>
              ...
              <div
                class="border border-gray-200 dark:border-gray-700 rounded-lg divide-y divide-gray-200 dark:divide-gray-700 max-h-40 overflow-y-auto"
              >
                <div
                  v-for="recipient in recipients"
                  :key="recipient.id"
      ...
      <UModal
        ...
        title="Export Envelope"
        description="Export the envelope in hex, base64, tree, or UR format."
      >
        <template #content>
          <div class="p-6">
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                >
                <div
                  class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-600 dark:text-gray-400 break-all max-h-40 overflow-y-auto"
                >
      ...
      <UModal
        ...
        description="Import an envelope from hex, base64, or UR data."
      >
        <template #content>
          <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
              ...
              <div>
                ...
                <UFieldGroup class="w-full">
                  <UButton
                    ...
                    @click="
                      () => {
                        importFormat = format as ExportFormat;
                      }
                    "
                  >
                    {{ format.toUpperCase() }}
      ...
      <UModal
        ...
        description="Choose a template to create a pre-structured envelope."
      >
        <template #content>
          <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
      ...
      <UModal
        ...
        title="Add Type"
        description="Add a semantic type classification to the envelope."
      >
        <template #content>
          <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Add Type</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Add a semantic type classification to the envelope (e.g., Person, Invoice,
              Certificate).
            </p>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Type Name</label
              >
              <UInput v-model="typeValue" placeholder="e.g., Person, Invoice, Certificate" />
            </div>
      ...
      <UModal
        v-model:open="showAttachmentModal"
        title="Add Attachment"
        description="Add vendor-specific metadata as an attachment."
      >
        <template #content>
          <div class="p-6">
            ...
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                  >Vendor ID</label
                >
                <UInput v-model="attachmentVendor" placeholder="e.g., com.example, org.mycompany" />
            ...
            <div class="flex justify-end gap-2 mt-6">
              ...
              <UButton
                color="primary"
                :disabled="!attachmentPayload.trim() || !attachmentVendor.trim()"
                @click="addAttachmentToNode"
              >
      ...
      <!-- Phase 3: Proof Modal -->
      <UModal
        v-model:open="showProofModal"
        title="Inclusion Proof"
        description="Generate a cryptographic proof of element inclusion."
      >
        <template #content>
          <div class="p-6 max-w-2xl">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Cryptographic proof that an element exists in the envelope without revealing the full
              contents.
            </p>
            ...
            <template v-if="proofTreeOutput">
              ...
              <div class="flex items-center gap-2 mb-4">
                <UBadge :color="proofVerified ? 'success' : 'error'" variant="soft" size="lg">
                  <UIcon
                    :name="proofVerified ? 'i-heroicons-check-circle' : 'i-heroicons-x-circle'"
                    class="w-4 h-4 mr-1"
                  />
              ...
              <div class="mb-4">
                ...
                >
                <div
                  class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-2 text-sm font-mono text-gray-700 dark:text-gray-300 break-all"
                >
                  {{ proofTargetDisplay }}
                </div>
              </div>
              ...
              <div class="mb-4">
                ...
                <div
                  class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-700 dark:text-gray-300 whitespace-pre-wrap break-all max-h-48 overflow-y-auto"
                >
                  {{ proofTreeOutput }}
                </div>
              </div>
              ...
              <div class="mb-4">
                ...
                <div
                  class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-700 dark:text-gray-300 break-all max-h-24 overflow-y-auto"
                >
                  {{ proofHexOutput }}
                </div>
              </div>
              ...
              <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                  >Root Digest (for verification)</label
                >
                <div
                  class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-2 text-xs font-mono text-gray-700 dark:text-gray-300 break-all"
                >
                  {{ digestOutput }}
                </div>
              </div>
              ...
              >
                ...
                <template #description>
                  Share the proof hex and root digest. The verifier can confirm the target element
                  exists without seeing the full envelope contents.
                </template>
              </UAlert>
            </template>
            ...
            <template v-else-if="proofOutput && !proofTreeOutput">
              <div
                class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-sm font-mono text-gray-700 dark:text-gray-300 whitespace-pre-wrap break-all"
              >
                {{ proofOutput }}
              </div>
            </template>
      ...
      <UModal
        ...
        title="QR Code"
        description="Scan this QR code to import the envelope on another device."
      >
        <template #content>
          <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
      ...
      <UModal
        ...
        title="Sign Envelope"
        description="Configure signing options for the envelope."
      >
        <template #content>
          <div class="p-6 min-w-[400px]">
            ...
            <div class="space-y-4">
                  ...
                  >
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="text-xs text-gray-500 dark:text-gray-400">
                        {{ key.algorithm }} &middot; {{ key.publicKeyHex.substring(0, 20) }}...
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                  >Signature Note (optional)</label
                >
                <UInput v-model="signNote" placeholder="e.g., Approved by Alice" />
                <p class="text-xs text-gray-500 mt-1">
                  Metadata attached to the signature (signed with the envelope).
                </p>
              </div>
            </div>
            ...
            <div class="flex justify-end gap-2 mt-6">
              ...
              >
                Sign{{
                  signSelectedKeyIds.length > 1 ? ` (${signSelectedKeyIds.length} keys)` : ""
                }}
              </UButton>
            </div>
          </div>
      ...
      <UModal
        v-model:open="showPasswordEncryptModal"
        title="Password Encryption"
        description="Encrypt the envelope with a password."
      >
        <template #content>
          <div class="p-6 min-w-[400px]">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <div class="text-xs text-blue-600 dark:text-blue-400 mt-1">
                  The password will be used to derive a symmetric key which encrypts the envelope
                  contents.
                </div>
              </div>
            </div>
      ...
      <UModal
        ...
        title="Seal Envelope"
        description="Sign-then-encrypt for authenticated confidential delivery."
      >
        <template #content>
          <div class="p-6 min-w-[400px]">
      ...
      <UModal
        v-model:open="showSskrSplitModal"
        title="SSKR Split"
        description="Split the envelope into secret shares using Shamir's Secret Sharing."
      >
        <template #content>
          <div class="p-6 max-h-[80vh] overflow-y-auto">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">SSKR Split</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Split this envelope into shares using Sharded Secret Key Reconstruction. Configure
              group thresholds to require shares from multiple parties.
            </p>

            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                  >Group Threshold</label
                >
      ...
      <UModal
        v-model:open="showSskrJoinModal"
        title="SSKR Join"
        description="Reconstruct an envelope from SSKR shares."
      >
        <template #content>
          <div class="p-6">
      ...
      <UModal
        ...
        description="Verify the cryptographic signature on this envelope."
      >
        <template #content>
          <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
      ...
      <UModal
        ...
        description="Step-by-step wizard to build a new envelope."
      >
        <template #content>
          <div class="p-6 min-w-[400px]">
            <!-- Progress indicator -->
            <div class="flex items-center justify-between mb-6">
              <div class="flex items-center gap-2">
                <div v-for="step in wizardTotalSteps" :key="step" class="flex items-center">
                  <div
                    :class="[
                  ...
                  v-for="type in [
                    {
                      ...
                      desc: 'Person or entity',
                    },
                    {
                      id: 'document',
                      label: 'Document',
                      icon: 'i-heroicons-document-text',
                      desc: 'Certificate or record',
                    },
                    {
                      id: 'custom',
                      label: 'Custom',
            ...
            <div v-if="wizardStep === 2">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  {{
                    wizardEnvelopeType === "identity"
                      ? "Name"
                      : wizardEnvelopeType === "document"
                        ? "Title"
                        : "Subject Value"
                  }}
                </label>
              ...
              <div class="space-y-3 max-h-64 overflow-y-auto">
                ...
                >
                  <UInput
                    ...
                    class="flex-1"
                  />
                  <UInput v-model="assertion.value" placeholder="Value" class="flex-1" />
                  <UButton
                    icon="i-heroicons-x-mark"
                    color="error"
                    variant="ghost"
            ...
            <div class="flex justify-between mt-6">
              <UButton v-if="wizardStep > 1" color="neutral" variant="ghost" @click="wizardBack">
              ...
              <div class="flex gap-2">
                ...
                <UButton v-if="wizardStep < wizardTotalSteps" color="primary" @click="wizardNext">
                  Next
                  <UIcon name="i-heroicons-arrow-right" class="w-4 h-4 ml-1" />
                </UButton>
                <UButton v-else color="primary" @click="wizardComplete">
                  <UIcon name="i-heroicons-sparkles" class="w-4 h-4 mr-1" />
                  Create Envelope
                </UButton>
              </div>
      ...
      <UModal
        v-model:open="showUnelideModal"
        title="Unelide"
        description="Restore an elided envelope by providing the original envelope data."
      >
        <template #content>
          <div class="p-6 max-w-lg">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
      ...
      <UModal
        v-model:open="showDecryptModal"
        title="Decrypt"
        description="Decrypt an encrypted envelope using a symmetric key or password."
      >
        <template #content>
          <div class="p-6 max-w-lg">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
            ...
            <div v-if="decryptMode === 'symmetric'" class="mb-4">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Symmetric Key (hex)</label
              >
              <UInput
                v-model="decryptKeyHex"
                placeholder="Enter key as hex..."
                size="sm"
                class="font-mono"
              />
            </div>
            ...
            <div v-else class="mb-4">
              ...
              <p class="text-xs text-gray-500 mt-1">
                The recipient's private key will be used to decrypt.
              </p>
            </div>
      ...
      <UModal
        v-model:open="showExpressionModal"
        :title="expressionMode.charAt(0).toUpperCase() + expressionMode.slice(1) + ' Builder'"
        description="Create an expression, request, response, or event envelope."
      >
        <template #content>
          <div class="p-6 max-w-lg">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
            ...
            <template v-if="expressionMode === 'expression' || expressionMode === 'request'">
              <div class="mb-4">
                ...
                <select
                  ...
                  class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
                >
                  <option v-for="f in knownFunctions" :key="f.value" :value="f.value">
                    {{ f.label }}
                  </option>
                </select>
              </div>
              ...
              <div class="mb-4">
                ...
                <div v-for="(p, i) in exprParams" :key="i" class="flex gap-2 mb-2">
                  ...
                  <select
                    v-model="p.valueType"
                    class="text-xs bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-1.5 py-1"
                  >
            ...
            <template v-if="expressionMode === 'response'">
              <div class="mb-4">
                ...
                <UInput
                  ...
                  :placeholder="
                    responseIsSuccess ? 'Result value (optional)...' : 'Error message (optional)...'
                  "
                  size="sm"
                />
              </div>
      ...
      <!-- Phase 5: Unseal Modal -->
      <UModal
        v-model:open="showUnsealModal"
        title="Unseal"
        description="Decrypt and verify a sealed envelope in one step."
      >
        <template #content>
          <div class="p-6 max-w-lg">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Unseal performs decrypt-then-verify: decrypts with the recipient's private key, then
            ...
            <div class="mb-4">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Sender's Signing Key (for verification)</label
              >
              <select
                v-model="unsealSignerKeyId"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              >
                <option v-for="k in signingKeys" :key="k.id" :value="k.id">
                  {{ k.name }} ({{ k.algorithm }})
                </option>
              </select>
            </div>
            ...
            <div class="mb-4">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Recipient (for decryption)</label
              >
              <select
                v-model="unsealRecipientId"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              >
                <option v-for="r in recipients" :key="r.id" :value="r.id">
                  {{ r.name }} ({{ r.scheme }})
            ...
            <div class="flex justify-end gap-2">
              ...
              <UButton
                color="primary"
                :disabled="!unsealSignerKeyId || !unsealRecipientId"
                @click="performUnseal"
              >
      ...
      <!-- Phase 5: Walk Replace Modal -->
      <UModal
        v-model:open="showWalkReplaceModal"
        title="Walk Replace"
        description="Replace nodes matching a digest with a new envelope."
      >
        <template #content>
          <div class="p-6 max-w-lg">
            ...
            <UAlert
              v-if="walkReplaceError"
              color="error"
              variant="soft"
              :title="walkReplaceError"
              class="mb-4"
            />

            <div class="mb-4">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Target Digest (hex, 64 chars)</label
              >
              <UInput
                v-model="walkReplaceDigestHex"
                placeholder="Enter digest hex..."
                size="sm"
                class="font-mono"
              />
            </div>
            ...
            <div class="mb-4">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Replacement Envelope (hex or UR)</label
              >
              <textarea
            ...
            <div class="flex justify-end gap-2">
              ...
              <UButton
                color="primary"
                :disabled="!walkReplaceDigestHex.trim() || !walkReplaceEnvelopeInput.trim()"
                @click="performWalkReplace"
              >
