...
import { Envelope, type TreeFormatOptions } from "@blockchaincommons/envelope/all";
import {
  PrivateKeyBase,
  SignatureScheme,
  EncapsulationScheme,
...
import type { ProvenanceMarkResolution } from "@blockchaincommons/provenance-mark";
...
useHead({
  ...
  meta: [
    {
      name: "description",
      content: "Create and inspect XID Documents with identity display and envelope output",
    },
  ],
});
...
const newKeyScheme = ref<"Ed25519" | "Schnorr" | "ECDSA" | "MLDSA44" | "MLDSA65" | "MLDSA87">(
  "Ed25519",
);
...
const newXIDResolution = ref<ProvenanceMarkResolution>("high");
...
const keyList = computed(() => {
  ...
  if (!xidDocument.value) return [] as readonly Key[];
...
const delegateList = computed(() => {
  ...
  if (!xidDocument.value) return [] as readonly Delegate[];
...
const serviceList = computed(() => {
  void docVersion.value;
  if (!xidDocument.value) return [] as readonly Service[];
...
const provenanceChainIdHex = computed(() => {
  ...
  if (!mark) return "";
  return Array.from(mark.chainId)
...
const attachmentList = computed(() => {
  ...
  for (const envelope of xidDocument.value.attachments) {
    const digestHex = envelope.digest().toHex();
    let vendor = "(unknown)";
    let conformsTo: string | undefined;
    let payloadPreview = "";
    try {
      vendor = envelope.attachmentVendor();
    } catch {
      /* ignore */
    }
    try {
      conformsTo = envelope.attachmentConformsTo();
    } catch {
      /* ignore */
    }
    try {
      payloadPreview = envelope.attachmentPayload().asText() ?? "(binary data)";
    } catch {
...
const edgeList = computed(() => {
  void docVersion.value;
  if (!xidDocument.value)
    return [] as { digestHex: string; envelope: Envelope; treePreview: string }[];
  const results: { digestHex: string; envelope: Envelope; treePreview: string }[] = [];
  for (const envelope of xidDocument.value.edges()) {
    const digestHex = envelope.digest().toHex();
    let treePreview = "";
    try {
...
// Reactive resolution methods list: the URIs flattened to strings so the
// template can compare and render them as ordinary text.
...
function createNewXID() {
  try {
    ...
    let genesis: XIDGenesis | undefined;
...
function updateIdentityDisplay() {
  ...
  xidUrString.value = xid.toUR().toString();
  ...
  if (inceptionKey) {
    ...
    inceptionKeyPermissions.value = Array.from(inceptionKey.allow);
  }
}
...
function updateOutput() {
  ...
  try {
    ...
    hexOutput.value = Array.from(bytes)
      .map((b) => b.toString(16).padStart(2, "0"))
      .join("");
    digestOutput.value = envelope.digest().toHex();
    envelopeUrOutput.value = envelope.toUR().toString();
    error.value = null;
  } catch (e) {
...
function addNewKey() {
  ...
  try {
    ...
    const usePrivateKeyBase =
      ["Ed25519", "Schnorr", "ECDSA"].includes(newKeyScheme.value) &&
      newKeyEncapsulation.value === "X25519";
    ...
    if (usePrivateKeyBase) {
      ...
      switch (newKeyScheme.value) {
        case "Ed25519":
          key = Key.from(pkb.ed25519PublicKeys(), { privateKeys: pkb.ed25519PrivateKeys() });
          break;
        case "Schnorr":
          key = Key.from(pkb.schnorrPublicKeys(), { privateKeys: pkb.schnorrPrivateKeys() });
          break;
        case "ECDSA":
          key = Key.from(pkb.ecdsaPublicKeys(), { privateKeys: pkb.ecdsaPrivateKeys() });
          break;
        default:
          key = Key.from(pkb.ed25519PublicKeys(), { privateKeys: pkb.ed25519PrivateKeys() });
      }
      ...
      const privateKeys = PrivateKeys.from({ signing: sigPriv, encapsulation: encPriv });
      const publicKeys = PublicKeys.from({ signing: sigPub, encapsulation: encPub });
      key = Key.from(publicKeys, { privateKeys });
    }

    for (const p of newKeyAllowPrivileges.value) {
      key.addAllow(p as Privilege);
    }
    for (const p of newKeyDenyPrivileges.value) {
      key.addDeny(p as Privilege);
    }
    ...
    toast.add({
      title: e instanceof Error ? e.message : "Failed to add key",
      color: "error",
      icon: "i-heroicons-exclamation-triangle",
    });
  }
...
function confirmRemoveKey(key: Key) {
  ...
  if (isKeyInception(key)) {
    toast.add({
      title: "Cannot remove the inception key",
      color: "error",
      icon: "i-heroicons-exclamation-triangle",
    });
    return;
...
function executeRemoveKey() {
  ...
  try {
    ...
    toast.add({
      title: e instanceof Error ? e.message : "Failed to remove key",
      color: "error",
      icon: "i-heroicons-exclamation-triangle",
    });
  }
...
function saveNickname(key: Key) {
  ...
  try {
    ...
    toast.add({
      title: e instanceof Error ? e.message : "Failed to update nickname",
      color: "error",
    });
  }
}
...
function startEditPermissions(key: Key) {
  ...
  editingPermsAllow.value = new Set<string>(key.allow);
  editingPermsDeny.value = new Set<string>(key.deny);
}
...
function addNewDelegate() {
  ...
  try {
    const controllerDoc = XIDDocument.random();
    const delegate = Delegate.from(controllerDoc);

    for (const p of newDelegateAllowPrivileges.value) {
      delegate.addAllow(p as Privilege);
    }
    for (const p of newDelegateDenyPrivileges.value) {
      delegate.addDeny(p as Privilege);
    }
    ...
    toast.add({
      title: e instanceof Error ? e.message : "Failed to add delegate",
      color: "error",
      icon: "i-heroicons-exclamation-triangle",
    });
  }
...
function executeRemoveDelegate() {
  ...
  try {
    ...
    toast.add({
      title: e instanceof Error ? e.message : "Failed to remove delegate",
      color: "error",
      icon: "i-heroicons-exclamation-triangle",
    });
  }
...
function startEditDelegatePermissions(delegate: Delegate) {
  ...
  editingDelegatePermsAllow.value = new Set<string>(delegate.allow);
  editingDelegatePermsDeny.value = new Set<string>(delegate.deny);
}

function saveDelegatePermissions(delegate: Delegate) {
  delegate.clearAllPermissions();
...
function openEditServiceModal(service: Service) {
  ...
  // The form binds strings: the URI as text and the references as hex.
  ...
  serviceFormKeyRefs.value = new Set(Array.from(service.keyReferences).map((r) => r.toHex()));
  serviceFormDelegateRefs.value = new Set(
    Array.from(service.delegateReferences).map((r) => r.toHex()),
  );
  serviceFormAllowPrivileges.value = new Set<string>(service.allow);
  serviceFormDenyPrivileges.value = new Set<string>(service.deny);
  serviceFormError.value = null;
  showServiceModal.value = true;
}
...
function saveService() {
  ...
  try {
    ...
    for (const ref of serviceFormKeyRefs.value) {
      service.addKeyReference(Reference.fromHex(ref));
    }

    for (const ref of serviceFormDelegateRefs.value) {
      service.addDelegateReference(Reference.fromHex(ref));
    }

    for (const p of serviceFormAllowPrivileges.value) {
      service.addAllow(p as Privilege);
    }
    for (const p of serviceFormDenyPrivileges.value) {
      service.addDeny(p as Privilege);
    }
...
function executeRemoveService() {
  ...
  try {
    ...
    toast.add({
      title: e instanceof Error ? e.message : "Failed to remove service",
      color: "error",
      icon: "i-heroicons-exclamation-triangle",
    });
  }
...
function advanceProvenance() {
  ...
  try {
    ...
    refreshDocument();
    toast.add({
      title: "Provenance mark advanced",
      color: "success",
...
function removeAttachment(envelope: Envelope) {
  ...
  try {
    ...
    toast.add({
      title: e instanceof Error ? e.message : "Failed to remove attachment",
      color: "error",
      icon: "i-heroicons-exclamation-triangle",
    });
  }
...
function addEdge() {
  ...
  try {
    const edgeEnvelope = decodeURWith(UR.parse(urString), Envelope.codec);
...
function removeEdge(envelope: Envelope) {
  ...
  try {
    ...
    toast.add({
      title: e instanceof Error ? e.message : "Failed to remove edge",
      color: "error",
      icon: "i-heroicons-exclamation-triangle",
    });
  }
...
function importDocument() {
  ...
  try {
    ...
    const password = importPassword.value
      ? new TextEncoder().encode(importPassword.value)
      : undefined;
    const verify: XIDVerifySignature = importVerifySignature.value ? "inception" : "none";
...
function executeExport() {
  ...
  try {
    ...
    let privateKeyOpts: XIDPrivateKeyOptions;
    switch (exportPrivateKeyOption.value) {
      case "Include":
        privateKeyOpts = "include";
        break;
      case "Elide":
        privateKeyOpts = "elide";
        break;
      case "Encrypt": {
        ...
        privateKeyOpts = {
          encrypt: new TextEncoder().encode(exportPrivateKeyPassword.value),
    ...
    let generatorOpts: XIDGeneratorOptions;
    switch (exportGeneratorOption.value) {
      case "Include":
        generatorOpts = "include";
        break;
      case "Elide":
        generatorOpts = "elide";
        break;
      case "Encrypt": {
        ...
        generatorOpts = {
          encrypt: new TextEncoder().encode(exportGeneratorPassword.value),
          method: exportKdfMethod.value,
        };
        break;
      }
    ...
    const envelope = doc.toEnvelope({
      privateKeys: privateKeyOpts,
      generator: generatorOpts,
      sign: exportSigningOption.value,
    });
    exportResult.value = envelope.toUR().toString();
...
function generateQRCode() {
  ...
  try {
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${svgSize} ${svgSize}" width="300" height="300">
      ...
      ${qrData
        .map((row: boolean[], y: number) =>
          row
            .map((cell: boolean, x: number) =>
              cell
                ? `<rect x="${x * scale}" y="${y * scale}" width="${scale}" height="${scale}" fill="#000000"/>`
                : "",
            )
            .join(""),
        )
...
function openQRModal() {
  if (!envelopeUrOutput.value) {
    toast.add({
      title: "Create an XID document first",
      color: "warning",
      icon: "i-heroicons-exclamation-triangle",
    });
    return;
...
function saveToHistory() {
  ...
  try {
    ...
    const state = envelope.toUR().toString();
...
function undo() {
  ...
  try {
    const envelope = decodeURWith(UR.parse(previousState), Envelope.codec);
...
function redo() {
  ...
  try {
    const envelope = decodeURWith(UR.parse(nextState), Envelope.codec);
    const doc = XIDDocument.fromEnvelope(envelope);
    xidDocument.value = doc;
    docVersion.value++;
    updateIdentityDisplay();
...
async function sharePublicly() {
  ...
  try {
    ...
    toast.add({
      title: "Public export copied to clipboard",
      color: "success",
      icon: "i-heroicons-share",
    });
  } catch (e) {
    toast.add({
      title: e instanceof Error ? e.message : "Failed to create public export",
      color: "error",
      icon: "i-heroicons-exclamation-triangle",
    });
  }
...
const xidTemplates: XIDTemplate[] = [
  ...
  },
  {
    ...
    create: () => {
      ...
      const controllerDoc = XIDDocument.random();
      const delegate = Delegate.from(controllerDoc);
      delegate.addAllow("Sign");
      delegate.addAllow("Auth");
  ...
  },
  {
    ...
    create: () => {
      ...
      const backupKey = Key.from(PublicKeys.from({ signing: sigPub, encapsulation: encPub }), {
        privateKeys: PrivateKeys.from({ signing: sigPriv, encapsulation: encPriv }),
      });
      backupKey.setNickname("PQ Backup");
      backupKey.addAllow("Sign");
      backupKey.addAllow("Auth");
  ...
  },
  {
    ...
    create: () => {
      ...
      const doc = XIDDocument.from({
        inceptionKey: { publicKeys: pkb.ecdsaPublicKeys(), privateKeys: pkb.ecdsaPrivateKeys() },
      });
      const inceptionKey = doc.inceptionKey;
      if (inceptionKey) {
      ...
      // Add a service
      const service = Service.from("https://api.example.com");
      service.setName("API Endpoint");
      service.setCapability("com.example.api");
      if (inceptionKey) service.addKeyReference(inceptionKey.reference);
      service.addAllow("Auth");
  ...
  <UDashboardPanel id="xid-builder">
    ...
    <template #body>
      <div class="flex h-full">
        ...
        >
          <div
            class="px-4 py-3 border-b border-gray-200 dark:border-gray-800/50 bg-gray-50 dark:bg-gray-900/50 flex items-center justify-between"
          >
          ...
          <div class="flex-1 overflow-auto p-4">
            ...
            <div v-if="!xidDocument" class="h-full flex flex-col items-center justify-center">
              <div class="text-center max-w-sm">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
                  Create a new XID Document to get started. A default Ed25519 inception key will be
                  generated automatically.
                </p>
                <div class="flex gap-2">
                  <UButton icon="i-heroicons-plus" color="primary" @click="openNewXIDModal">
                    Create XID Document
                  </UButton>
                  <UButton
                    icon="i-heroicons-squares-2x2"
            ...
            <div v-else class="space-y-4">
              ...
              <div>
                ...
                <div class="space-y-2">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <code
                      class="flex-1 text-xs bg-gray-100 dark:bg-gray-800/50 p-1.5 rounded font-mono break-all text-gray-700 dark:text-gray-300"
                      >{{ xidUrString }}</code
                    >
                    <UButton
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <code
                      class="flex-1 text-xs bg-gray-100 dark:bg-gray-800/50 p-1.5 rounded font-mono break-all text-gray-600 dark:text-gray-400"
                      >{{ xidHex }}</code
                    >
                    <UButton
                      icon="i-heroicons-clipboard-document"
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <code
                      class="flex-1 text-xs bg-gray-100 dark:bg-gray-800/50 p-1.5 rounded font-mono text-gray-700 dark:text-gray-300"
                      >{{ xidBytewords }}</code
                    >
                    <UButton
                      icon="i-heroicons-clipboard-document"
              ...
              <div>
                ...
                >
                  <div class="flex items-center gap-2">
                    <UBadge v-if="inceptionKeyIsInception" color="success" variant="soft" size="xs"
                      >Inception</UBadge
                    >
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    >
                    <code
                      class="flex-1 text-xs font-mono text-gray-600 dark:text-gray-400 break-all"
                      >{{ inceptionKeyReference }}</code
                    >
                    <UButton
                      icon="i-heroicons-clipboard-document"
                      size="xs"
                  ...
                  <div
                    v-if="inceptionKeyPermissions.length > 0"
              ...
              <div>
                <div
                  ...
                  @click="keySectionCollapsed = !keySectionCollapsed"
                >
                  <div class="flex items-center gap-2">
                    <UIcon
                      :name="
                  ...
                  >
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <UTooltip v-if="!isKeyInception(keyItem)" text="Remove key">
                        <UButton
                          ...
                          color="error"
                          variant="ghost"
                          @click="confirmRemoveKey(keyItem)"
                        />
                      </UTooltip>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <template v-else>
                        ...
                        <UButton
                          ...
                          color="neutral"
                          variant="ghost"
                          @click="startEditNickname(keyItem)"
                        />
                      </template>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <UButton
                        ...
                        color="neutral"
                        variant="ghost"
                        @click="copyToClipboard(getKeyRefHex(keyItem), 'Key reference')"
                      />
                    </div>
                    ...
                    <div v-if="editingPermsKeyRef !== getKeyRefHex(keyItem)">
                      <div class="flex items-center gap-1 flex-wrap">
                        ...
                        >
                        <UBadge
                          v-for="perm in Array.from(keyItem.allow)"
                          :key="'a-' + perm"
                          color="success"
                          variant="soft"
                          size="xs"
                          >{{ perm }}</UBadge
                        >
                        <span v-if="keyItem.allow.size === 0" class="text-xs text-gray-400 italic"
                          >none</span
                        >
                      </div>
                      <div
                        v-if="keyItem.deny.size > 0"
                        class="flex items-center gap-1 flex-wrap mt-1"
                      >
                        <span class="text-xs text-gray-500 dark:text-gray-400 shrink-0">Deny:</span>
                        <UBadge
                          v-for="perm in Array.from(keyItem.deny)"
                          :key="'d-' + perm"
                          color="error"
                          variant="soft"
                          size="xs"
                      ...
                      <UButton
                        ...
                        @click="startEditPermissions(keyItem)"
                      />
                    </div>
                    ...
                    <div v-else class="space-y-2">
                      <div
                        class="grid grid-cols-[1fr_auto_auto] gap-x-4 gap-y-1 text-xs bg-white dark:bg-gray-900/50 rounded p-2"
                      >
                        ...
                        <template v-for="priv in ALL_PRIVILEGES" :key="priv">
                          ...
                          <div class="text-center">
                            <input
                              type="checkbox"
                              :checked="editingPermsAllow.has(priv)"
                              @change="togglePrivilege(editingPermsAllow, priv)"
                            />
                          </div>
                          <div class="text-center">
                            <input
                              type="checkbox"
                              :checked="editingPermsDeny.has(priv)"
                              @change="togglePrivilege(editingPermsDeny, priv)"
                            />
                          </div>
                        </template>
                      </div>
                    ...
                    <div>
                      <div v-if="Array.from(keyItem.endpoints).length > 0" class="space-y-1">
                        ...
                        <div
                          v-for="ep in Array.from(keyItem.endpoints).map((u) => u.toString())"
                          :key="ep"
                        ...
                        >
                          ...
                          <UButton
                            ...
                            color="error"
                            variant="ghost"
                            @click="removeEndpointFromKey(keyItem, ep)"
                          />
                        </div>
                      </div>
                      <div
                        v-if="addingEndpointKeyRef === getKeyRefHex(keyItem)"
                      ...
                      >
                        ...
                        <UButton
                          ...
                          color="success"
                          variant="ghost"
                          @click="addEndpointToKey(keyItem)"
                        />
                        <UButton
                          icon="i-heroicons-x-mark"
                          size="xs"
              ...
              <div>
                <div
                  class="flex items-center justify-between cursor-pointer"
                  @click="delegateSectionCollapsed = !delegateSectionCollapsed"
                >
                  <div class="flex items-center gap-2">
                    <UIcon
                      :name="
                        delegateSectionCollapsed
                          ? 'i-heroicons-chevron-right'
                          : 'i-heroicons-chevron-down'
                      "
                      class="w-3.5 h-3.5 text-gray-400"
                  ...
                  <UButton
                    ...
                    color="neutral"
                    variant="ghost"
                    @click.stop="openAddDelegateModal"
                  />
                </div>

                <div v-if="!delegateSectionCollapsed" class="mt-2 space-y-2">
                  <div v-if="delegateList.length === 0" class="text-xs text-gray-400 italic pl-5">
                    No delegates
                  </div>
                  <div
                  ...
                  >
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <UTooltip text="Remove delegate">
                        <UButton
                          ...
                          color="error"
                          variant="ghost"
                          @click="confirmRemoveDelegate(delegateItem)"
                        />
                      </UTooltip>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <code
                        class="flex-1 text-xs font-mono text-gray-600 dark:text-gray-400 truncate"
                        >{{ getDelegateXidHex(delegateItem) }}</code
                      >
                      <UButton
                        icon="i-heroicons-clipboard-document"
                        size="xs"
                        color="neutral"
                        variant="ghost"
                        @click="copyToClipboard(getDelegateXidHex(delegateItem), 'Delegate XID')"
                      />
                    </div>
                    ...
                    <div v-if="editingDelegatePermsRef !== getDelegateXidHex(delegateItem)">
                      <div class="flex items-center gap-1 flex-wrap">
                        ...
                        >
                        <UBadge
                          v-for="perm in Array.from(delegateItem.allow)"
                          :key="'da-' + perm"
                          color="success"
                          variant="soft"
                          size="xs"
                        ...
                        >
                        <span
                          v-if="delegateItem.allow.size === 0"
                          class="text-xs text-gray-400 italic"
                          >none</span
                        >
                      </div>
                      <div
                        v-if="delegateItem.deny.size > 0"
                        class="flex items-center gap-1 flex-wrap mt-1"
                      >
                        <span class="text-xs text-gray-500 dark:text-gray-400 shrink-0">Deny:</span>
                        <UBadge
                          v-for="perm in Array.from(delegateItem.deny)"
                          :key="'dd-' + perm"
                          color="error"
                          variant="soft"
                          size="xs"
                      ...
                      <UButton
                        ...
                        variant="ghost"
                        label="Edit permissions"
                        @click="startEditDelegatePermissions(delegateItem)"
                      />
                    </div>
                    ...
                    <div v-else class="space-y-2">
                      ...
                      >
                        ...
                        <template v-for="priv in ALL_PRIVILEGES" :key="priv">
                          ...
                          <div class="text-center">
                            <input
                              type="checkbox"
                              :checked="editingDelegatePermsAllow.has(priv)"
                              @change="togglePrivilege(editingDelegatePermsAllow, priv)"
                            />
                          </div>
                          <div class="text-center">
                            <input
                              type="checkbox"
                              :checked="editingDelegatePermsDeny.has(priv)"
                              @change="togglePrivilege(editingDelegatePermsDeny, priv)"
                            />
                          </div>
                        </template>
                      </div>
                      <div class="flex gap-1">
                        <UButton
                          ...
                          color="primary"
                          variant="soft"
                          @click="saveDelegatePermissions(delegateItem)"
                          >Save</UButton
                        >
                        <UButton
                          size="xs"
                          color="neutral"
                          variant="ghost"
                          @click="cancelEditDelegatePermissions"
                          >Cancel</UButton
                        >
                      </div>
                    </div>
              ...
              <div>
                <div
                  class="flex items-center justify-between cursor-pointer"
                  @click="serviceSectionCollapsed = !serviceSectionCollapsed"
                >
                  <div class="flex items-center gap-2">
                    <UIcon
                      :name="
                        serviceSectionCollapsed
                          ? 'i-heroicons-chevron-right'
                          : 'i-heroicons-chevron-down'
                      "
                      class="w-3.5 h-3.5 text-gray-400"
                  ...
                  <UButton
                    ...
                    color="neutral"
                    variant="ghost"
                    @click.stop="openAddServiceModal"
                  />
                </div>

                <div v-if="!serviceSectionCollapsed" class="mt-2 space-y-2">
                  <div v-if="serviceList.length === 0" class="text-xs text-gray-400 italic pl-5">
                    No services
                  </div>
                  <div
                    v-for="serviceItem in serviceList"
                    :key="serviceItem.uri.toString()"
                    class="bg-gray-50 dark:bg-gray-800/30 rounded-lg p-3 space-y-2 border border-gray-200 dark:border-gray-700"
                  >
                    <!-- Header: badges + actions -->
                    <div class="flex items-center justify-between">
                      <div class="flex items-center gap-1.5 flex-wrap">
                        ...
                        <UBadge v-if="serviceItem.name" color="info" variant="soft" size="xs">{{
                          serviceItem.name
                        }}</UBadge>
                      </div>
                      <div class="flex items-center gap-1">
                        <UTooltip text="Edit service">
                          <UButton
                            ...
                            color="neutral"
                            variant="ghost"
                            @click="openEditServiceModal(serviceItem)"
                          />
                        </UTooltip>
                        <UTooltip text="Remove service">
                          <UButton
                            ...
                            color="error"
                            variant="ghost"
                            @click="confirmRemoveService(serviceItem)"
                          />
                        </UTooltip>
                      </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <UButton
                        ...
                        color="neutral"
                        variant="ghost"
                        @click="copyToClipboard(serviceItem.uri.toString(), 'Service URI')"
                      />
                    </div>
                    ...
                    <div v-if="serviceItem.capability" class="flex items-center gap-2">
                      ...
                      <code
                        class="flex-1 text-xs font-mono text-gray-600 dark:text-gray-400 truncate"
                        >{{ serviceItem.capability }}</code
                      >
                    </div>

                    <!-- Key References -->
                    <div
                      v-if="serviceItem.keyReferences.size > 0"
                      class="flex items-center gap-1 flex-wrap"
                    >
                      <span class="text-xs text-gray-500 dark:text-gray-400 shrink-0">Keys:</span>
                      <UBadge
                        v-for="ref in Array.from(serviceItem.keyReferences).map((r) => r.toHex())"
                        :key="'sk-' + ref"
                        color="neutral"
                        variant="soft"
                        size="xs"
                    ...
                    <!-- Delegate References -->
                    <div
                      v-if="serviceItem.delegateReferences.size > 0"
                      class="flex items-center gap-1 flex-wrap"
                    >
                      <span class="text-xs text-gray-500 dark:text-gray-400 shrink-0"
                        >Delegates:</span
                      >
                      <UBadge
                        v-for="ref in Array.from(serviceItem.delegateReferences).map((r) =>
                          r.toHex(),
                        )"
                        :key="'sdr-' + ref"
                        color="info"
                    ...
                    <div class="flex items-center gap-1 flex-wrap">
                      ...
                      <UBadge
                        v-for="perm in Array.from(serviceItem.allow)"
                        :key="'sa-' + perm"
                        color="success"
                        variant="soft"
                        size="xs"
                        >{{ perm }}</UBadge
                      >
                      <span v-if="serviceItem.allow.size === 0" class="text-xs text-gray-400 italic"
                        >none</span
                      >
                    </div>
                    <div v-if="serviceItem.deny.size > 0" class="flex items-center gap-1 flex-wrap">
                      <span class="text-xs text-gray-500 dark:text-gray-400 shrink-0">Deny:</span>
                      <UBadge
                        v-for="perm in Array.from(serviceItem.deny)"
                        :key="'sdp-' + perm"
                        color="error"
                        variant="soft"
                        size="xs"
              ...
              <div>
                <div
                  class="flex items-center justify-between cursor-pointer"
                  @click="provenanceSectionCollapsed = !provenanceSectionCollapsed"
                >
                  <div class="flex items-center gap-2">
                    <UIcon
                      :name="
                        provenanceSectionCollapsed
                          ? 'i-heroicons-chevron-right'
                          : 'i-heroicons-chevron-down'
                      "
                      class="w-3.5 h-3.5 text-gray-400"
                    ...
                    <UBadge v-if="provenanceMark" color="success" variant="soft" size="xs"
                      >Seq {{ provenanceMark.seq }}</UBadge
                    >
                ...
                <div v-if="!provenanceSectionCollapsed" class="mt-2">
                  <div v-if="!provenanceMark" class="text-xs text-gray-400 italic pl-5">
                    No provenance mark. Create a new document with provenance enabled to set a
                    genesis mark.
                  </div>
                  <div
                    v-else
                  ...
                  >
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <UTooltip v-if="generatorStatus === 'Included'" text="Advance provenance">
                        <UButton
                          ...
                          color="primary"
                          variant="ghost"
                          @click.stop="openAdvanceProvenanceModal"
                        />
                      </UTooltip>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <UBadge v-if="provenanceMark.seq === 0" color="info" variant="soft" size="xs"
                        >Genesis</UBadge
                      >
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <UButton
                        ...
                        color="neutral"
                        variant="ghost"
                        @click="copyToClipboard(provenanceChainIdHex, 'Chain ID')"
                      />
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-xs text-gray-700 dark:text-gray-300">{{
                        provenanceMark.date.toISOString()
                      }}</span>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <code class="text-xs font-mono text-gray-600 dark:text-gray-400">{{
                        provenanceMark.idBytewords({ prefix: true })
                      }}</code>
                    </div>
                  </div>
                </div>
              ...
              <div>
                <div
                  class="flex items-center justify-between cursor-pointer"
                  @click="attachmentSectionCollapsed = !attachmentSectionCollapsed"
                >
                  <div class="flex items-center gap-2">
                    <UIcon
                      :name="
                        attachmentSectionCollapsed
                          ? 'i-heroicons-chevron-right'
                          : 'i-heroicons-chevron-down'
                      "
                      class="w-3.5 h-3.5 text-gray-400"
                  ...
                  <UButton
                    ...
                    color="neutral"
                    variant="ghost"
                    @click.stop="openAddAttachmentModal"
                  />
                </div>

                <div v-if="!attachmentSectionCollapsed" class="mt-2 space-y-2">
                  <div v-if="attachmentList.length === 0" class="text-xs text-gray-400 italic pl-5">
                    No attachments
                  </div>
                  <div
                  ...
                  >
                    <div class="flex items-center justify-between">
                      ...
                      <UTooltip text="Remove attachment">
                        <UButton
                          ...
                          color="error"
                          variant="ghost"
                          @click="removeAttachment(att.envelope)"
                        />
                      </UTooltip>
                    </div>
                    <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <code
                        class="flex-1 text-xs font-mono text-gray-600 dark:text-gray-400 truncate"
                        >{{ att.digestHex.substring(0, 16) }}...</code
                      >
                    </div>
                  </div>
                </div>
              ...
              <div>
                <div
                  class="flex items-center justify-between cursor-pointer"
                  @click="edgeSectionCollapsed = !edgeSectionCollapsed"
                >
                  <div class="flex items-center gap-2">
                    <UIcon
                      :name="
                        edgeSectionCollapsed
                          ? 'i-heroicons-chevron-right'
                          : 'i-heroicons-chevron-down'
                      "
                      class="w-3.5 h-3.5 text-gray-400"
                  ...
                  <UButton
                    ...
                    color="neutral"
                    variant="ghost"
                    @click.stop="openAddEdgeModal"
                  />
                </div>

                <div v-if="!edgeSectionCollapsed" class="mt-2 space-y-2">
                  <div v-if="edgeList.length === 0" class="text-xs text-gray-400 italic pl-5">
                    No edges
                  </div>
                  <div
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <UTooltip text="Remove edge">
                        <UButton
                          ...
                          color="error"
                          variant="ghost"
                          @click="removeEdge(edge.envelope)"
                        />
                      </UTooltip>
                    </div>
                    <pre
                      class="text-xs font-mono text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-900/50 rounded p-2 overflow-x-auto whitespace-pre max-h-24 overflow-y-auto"
                      >{{ edge.treePreview }}</pre>
                    <div class="flex items-center gap-2">
                      <span class="text-xs text-gray-500 dark:text-gray-400 shrink-0">Digest:</span>
                      <code
                        class="flex-1 text-xs font-mono text-gray-600 dark:text-gray-400 truncate"
                        >{{ edge.digestHex.substring(0, 16) }}...</code
                      >
                    </div>
                  </div>
                </div>
              ...
              <div>
                <div
                  class="flex items-center justify-between cursor-pointer"
                  @click="resolutionSectionCollapsed = !resolutionSectionCollapsed"
                >
                  <div class="flex items-center gap-2">
                    <UIcon
                      :name="
                        resolutionSectionCollapsed
                          ? 'i-heroicons-chevron-right'
                          : 'i-heroicons-chevron-down'
                      "
                      class="w-3.5 h-3.5 text-gray-400"
                  ...
                  <UButton
                    ...
                    color="neutral"
                    variant="ghost"
                    @click.stop="startAddResolutionMethod"
                  />
                </div>

                <div v-if="!resolutionSectionCollapsed" class="mt-2 space-y-2">
                  <div
                    v-if="resolutionMethodList.length === 0 && !addingResolutionMethod"
                    class="text-xs text-gray-400 italic pl-5"
                  >
                    No resolution methods
                  </div>
                  <div
                  ...
                  >
                    ...
                    <UButton
                      ...
                      color="neutral"
                      variant="ghost"
                      @click="copyToClipboard(method, 'Resolution URI')"
                    />
                    <UButton
                      icon="i-heroicons-x-mark"
                      size="xs"
                      color="error"
                      variant="ghost"
                      @click="removeResolutionMethod(method)"
                    />
                  </div>
                  <div v-if="addingResolutionMethod" class="flex items-center gap-1 pl-5">
                    ...
                    <UButton
                      ...
                      color="neutral"
                      variant="ghost"
                      @click="cancelAddResolutionMethod"
                    />
                  </div>
                </div>
              </div>
              ...
              <div v-if="envelopeUrOutput">
                ...
                <code
                  class="block text-xs bg-gray-100 dark:bg-gray-800/50 p-2 rounded font-mono break-all text-gray-600 dark:text-gray-400 max-h-24 overflow-y-auto"
                  >{{ envelopeUrOutput }}</code
                >
              </div>
            </div>
          </div>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
          <div
            class="px-4 py-3 border-b border-gray-200 dark:border-gray-800/50 bg-gray-50 dark:bg-gray-900/50"
          >
          ...
          <div class="flex-1 overflow-auto p-4 space-y-4">
            ...
            <template v-else>
              ...
              <div v-if="digestOutput">
                ...
                <div
                  class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-2 text-xs font-mono text-gray-600 dark:text-gray-400 break-all"
                >
                  {{ digestOutput }}
                </div>
              </div>
              ...
              <div v-if="treeOutput">
                ...
                <pre
                  class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-800 dark:text-gray-200 overflow-x-auto whitespace-pre"
                  >{{ treeOutput }}</pre>
              </div>
              ...
              <div v-if="notationOutput || hexOutput || diagnosticOutput">
                <div class="flex items-center justify-between mb-2">
                  ...
                  >
                    {{
                      envelopeOutputMode === "notation"
                        ? "Envelope Notation"
                        : envelopeOutputMode === "hex"
                          ? "CBOR (Hex)"
                          : "CBOR Diagnostic"
                    }}
                    <UBadge
                      v-if="envelopeOutputMode === 'hex' && hexOutput"
                      color="neutral"
                      variant="soft"
                      size="xs"
                      class="ml-1"
                      >{{ hexOutput.length / 2 }} bytes</UBadge
                    >
                  </h4>
                  <div class="flex items-center gap-1">
                    <select
                ...
                <pre
                  v-if="envelopeOutputMode === 'notation' && notationOutput"
                  class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-800 dark:text-gray-200 overflow-x-auto whitespace-pre"
                  >{{ notationOutput }}</pre>
                <div
                  v-else-if="envelopeOutputMode === 'hex' && hexOutput"
                  class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-600 dark:text-gray-400 break-all max-h-48 overflow-y-auto"
                >
                  {{ hexOutput }}
                </div>
                <pre
                  v-else-if="envelopeOutputMode === 'diag' && diagnosticOutput"
                  class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-800 dark:text-gray-200 overflow-x-auto whitespace-pre"
                  >{{ diagnosticOutput }}</pre>
              </div>
            </template>
          </div>
        </div>
      ...
      <UModal
        ...
        title="QR Code"
        description="Scan this QR code to import the XID on another device."
      >
        <template #content>
          <div class="p-6">
            ...
            <div v-if="qrCodeDataUrl" class="flex flex-col items-center">
              ...
              <p class="text-xs text-gray-500 text-center mb-4">Format: UR (Uniform Resource)</p>
      ...
      <UModal
        ...
        title="Add Key"
        description="Generate a new cryptographic key for this XID document."
      >
        <template #content>
          <div class="p-6 space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Encapsulation Algorithm</label
              >
              <select
                v-model="newKeyEncapsulation"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              >
                <optgroup label="Classical">
            ...
            <div>
              ...
              >
              <div
                class="grid grid-cols-[1fr_auto_auto] gap-x-4 gap-y-1 text-xs bg-gray-50 dark:bg-gray-800/30 rounded-lg p-3"
              >
                <div class="font-medium text-gray-500">Privilege</div>
                <div class="font-medium text-gray-500 text-center">Allow</div>
                <div class="font-medium text-gray-500 text-center">Deny</div>
                <template v-for="priv in ALL_PRIVILEGES" :key="priv">
                  ...
                  <div class="text-center">
                    <input
                      type="checkbox"
                      :checked="newKeyAllowPrivileges.has(priv)"
                      @change="togglePrivilege(newKeyAllowPrivileges, priv)"
                    />
                  </div>
                  <div class="text-center">
                    <input
                      type="checkbox"
                      :checked="newKeyDenyPrivileges.has(priv)"
                      @change="togglePrivilege(newKeyDenyPrivileges, priv)"
                    />
                  </div>
                </template>
              </div>
      ...
      <!-- Remove Key Confirmation -->
      <UModal
        v-model:open="showRemoveKeyConfirm"
        title="Remove Key"
        description="Confirm key removal from the XID document."
      >
        <template #content>
          <div class="p-6">
            ...
            <div
              v-if="removeKeyTarget"
              class="text-xs font-mono bg-gray-100 dark:bg-gray-800/50 rounded p-2 mb-4 text-gray-600 dark:text-gray-400"
            >
            ...
            <div class="flex justify-end gap-2">
              <UButton
                ...
                @click="
                  () => {
                    showRemoveKeyConfirm = false;
                  }
                "
                >Cancel</UButton
              >
              <UButton color="error" :disabled="!!removeKeyError" @click="executeRemoveKey"
                >Remove</UButton
              >
            </div>
          </div>
      ...
      <!-- Add Delegate Modal -->
      <UModal
        v-model:open="showAddDelegateModal"
        title="Add Delegate"
        description="Add a new delegate to this XID document."
      >
        <template #content>
          <div class="p-6 space-y-4">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Add Delegate</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400">
              A new XID document will be generated for the delegate. Configure its permissions
              below.
            </p>
              ...
              >
                ...
                <template v-for="priv in ALL_PRIVILEGES" :key="priv">
                  ...
                  <div class="text-center">
                    <input
                      type="checkbox"
                      :checked="newDelegateAllowPrivileges.has(priv)"
                      @change="togglePrivilege(newDelegateAllowPrivileges, priv)"
                    />
                  </div>
                  <div class="text-center">
                    <input
                      type="checkbox"
                      :checked="newDelegateDenyPrivileges.has(priv)"
                      @change="togglePrivilege(newDelegateDenyPrivileges, priv)"
                    />
                  </div>
                </template>
              </div>
      ...
      <!-- Remove Delegate Confirmation -->
      <UModal
        v-model:open="showRemoveDelegateConfirm"
        title="Remove Delegate"
        description="Confirm delegate removal from the XID document."
      >
        <template #content>
          <div class="p-6">
            ...
            <div
              v-if="removeDelegateTarget"
              class="text-xs font-mono bg-gray-100 dark:bg-gray-800/50 rounded p-2 mb-4 text-gray-600 dark:text-gray-400"
            >
              {{ getDelegateXidHex(removeDelegateTarget) }}
            </div>
            <UAlert
              v-if="removeDelegateError"
              color="error"
              variant="soft"
              icon="i-heroicons-exclamation-triangle"
              :title="removeDelegateError"
              class="mb-4"
            />
            <div class="flex justify-end gap-2">
              <UButton
                ...
                @click="
                  () => {
                    showRemoveDelegateConfirm = false;
                  }
                "
                >Cancel</UButton
              >
              <UButton
                color="error"
                :disabled="!!removeDelegateError"
                @click="executeRemoveDelegate"
                >Remove</UButton
              >
            </div>
          </div>
      ...
      <UModal
        ...
        :title="serviceModalMode === 'add' ? 'Add Service' : 'Edit Service'"
        :description="
          serviceModalMode === 'add' ? 'Add a new service endpoint.' : 'Edit service configuration.'
        "
      >
        <template #content>
          <div class="p-6 space-y-4">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ serviceModalMode === "add" ? "Add Service" : "Edit Service" }}
            </h3>
            ...
            <div>
              ...
              >
              <input
                v-model="serviceFormName"
                type="text"
                placeholder="e.g., Messaging Service"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              />
            </div>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Capability (optional)</label
              >
              <input
                v-model="serviceFormCapability"
                type="text"
                placeholder="e.g., com.example.messaging"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              />
            </div>
            ...
            <div v-if="keyList.length > 0">
              ...
              <div class="space-y-1 bg-gray-50 dark:bg-gray-800/30 rounded-lg p-3">
                <div
                  v-for="keyItem in keyList"
                  :key="'svc-key-' + getKeyRefHex(keyItem)"
                ...
                >
                  ...
                  <span class="text-xs text-gray-700 dark:text-gray-300">{{
                    keyItem.nickname || getKeyRefHex(keyItem).substring(0, 12) + "..."
                  }}</span>
                  <UBadge v-if="isKeyInception(keyItem)" color="success" variant="soft" size="xs"
                    >Inception</UBadge
                  >
                </div>
              </div>
            ...
            <div v-if="delegateList.length > 0">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"
                >Delegate References</label
              >
              <div class="space-y-1 bg-gray-50 dark:bg-gray-800/30 rounded-lg p-3">
                <div
                  v-for="delegateItem in delegateList"
                  :key="'svc-del-' + getDelegateXidHex(delegateItem)"
                  class="flex items-center gap-2"
                >
                  <input
                    type="checkbox"
                    ...
                    @change="
                      togglePrivilege(serviceFormDelegateRefs, getDelegateRefHex(delegateItem))
                    "
                  />
                  <UBadge color="info" variant="soft" size="xs">Delegate</UBadge>
                  <span class="text-xs text-gray-700 dark:text-gray-300 font-mono"
                    >{{ getDelegateXidHex(delegateItem).substring(0, 12) }}...</span
                  >
                </div>
              </div>
            </div>
              ...
              >
                ...
                <template v-for="priv in ALL_PRIVILEGES" :key="priv">
                  ...
                  <div class="text-center">
                    <input
                      type="checkbox"
                      :checked="serviceFormAllowPrivileges.has(priv)"
                      @change="togglePrivilege(serviceFormAllowPrivileges, priv)"
                    />
                  </div>
                  <div class="text-center">
                    <input
                      type="checkbox"
                      :checked="serviceFormDenyPrivileges.has(priv)"
                      @change="togglePrivilege(serviceFormDenyPrivileges, priv)"
                    />
                  </div>
                </template>
              </div>
      ...
      <!-- Remove Service Confirmation -->
      <UModal
        v-model:open="showRemoveServiceConfirm"
        title="Remove Service"
        description="Confirm service removal from the XID document."
      >
        <template #content>
          <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
      ...
      <!-- New XID Modal -->
      <UModal
        v-model:open="showNewXIDModal"
        title="New XID Document"
        description="Create a new XID document with optional provenance."
      >
        <template #content>
          <div class="p-6 space-y-4">
            ...
            <div class="flex items-center gap-3">
              <input
                ...
                v-model="newXIDWithProvenance"
                type="checkbox"
                class="rounded"
              />
              <label
                for="provenance-toggle"
                class="text-sm font-medium text-gray-700 dark:text-gray-300"
                >Create with provenance (genesis mark)</label
              >
            </div>

            <div
              v-if="newXIDWithProvenance"
              class="space-y-3 pl-2 border-l-2 border-primary-200 dark:border-primary-800"
            >
              <!-- Method -->
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                ...
                >
                <select
                  v-model="newXIDProvenanceMethod"
                  class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
                >
              ...
              <div v-if="newXIDProvenanceMethod === 'passphrase'">
                ...
                >
                <input
                  v-model="newXIDPassphrase"
                  type="password"
                  placeholder="Enter a passphrase for the provenance chain"
                  class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
                />
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Used to derive the provenance chain key. Keep this secret.
                </p>
              </div>

              <div v-else>
                <p class="text-xs text-gray-500 dark:text-gray-400">
                  A random 32-byte seed will be generated for the provenance chain.
                </p>
              </div>
              ...
              <div>
                ...
                >
                <select
                  v-model="newXIDResolution"
                  class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
                >
                  <option value="low">Low (16 bytes)</option>
                  <option value="medium">Medium (32 bytes)</option>
                  <option value="quartile">Quartile (58 bytes)</option>
                  <option value="high">High (106 bytes)</option>
                </select>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Higher resolution provides more precise date encoding and stronger chain
                  verification.
                </p>
              </div>
      ...
      <UModal
        v-model:open="showAdvanceProvenanceModal"
        title="Advance Provenance"
        description="Generate the next provenance mark in the chain."
      >
        <template #content>
          <div class="p-6 space-y-4">
            ...
            <UAlert
              v-if="advanceProvenanceError"
              color="error"
              variant="soft"
              icon="i-heroicons-exclamation-triangle"
              :title="advanceProvenanceError"
            />

            <div
              v-if="provenanceMark"
              class="bg-gray-50 dark:bg-gray-800/30 rounded-lg p-3 space-y-1 text-xs"
            >
              <div class="flex items-center gap-2">
            ...
            <div class="flex justify-end gap-2 pt-2">
              <UButton
                ...
                @click="
                  () => {
                    showAdvanceProvenanceModal = false;
                  }
                "
                >Cancel</UButton
              >
      ...
      <UModal
        v-model:open="showAddAttachmentModal"
        title="Add Attachment"
        description="Attach vendor-specific metadata to this XID document."
      >
        <template #content>
          <div class="p-6 space-y-4">
            ...
            <div>
              ...
              >
              <input
                v-model="attachmentFormVendor"
                type="text"
                placeholder="com.example"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              />
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Reverse domain name identifying the attachment format owner.
              </p>
            </div>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Conforms To (optional)</label
              >
              <input
                v-model="attachmentFormConformsTo"
                type="text"
                placeholder="https://schema.org/Thing"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              />
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                URI identifying the structure/schema of the payload.
              </p>
            </div>
            ...
            <div>
              ...
              >
              <textarea
                v-model="attachmentFormPayload"
                rows="3"
                placeholder="Attachment data..."
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 resize-none"
              />
            </div>
      ...
      <UModal
        ...
        title="Add Edge"
        description="Add a verifiable relationship edge to this XID document."
      >
        <template #content>
          <div class="p-6 space-y-4">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Add Edge</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400">
              Paste an edge envelope UR string. The edge should contain isA, source, and target
              assertions.
            </p>
            ...
            <div>
              ...
              >
              <textarea
                v-model="edgeFormUrString"
                rows="3"
                placeholder="ur:envelope/..."
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 resize-none font-mono"
              />
            </div>
      ...
      <UModal
        ...
        title="Import XID Document"
        description="Import an XID document from an envelope UR string."
      >
        <template #content>
          <div class="p-6 space-y-4">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Import XID Document</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400">
              Paste an envelope UR string containing an XID document. This will replace the current
              document.
            </p>
            ...
            <div>
              ...
              <input
                ...
                type="password"
                placeholder="For decrypting private keys or generator"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              />
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Required if private keys or generator are encrypted.
              </p>
            </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <label
                for="import-verify"
                class="text-sm font-medium text-gray-700 dark:text-gray-300"
                >Verify inception signature</label
              >
            </div>
      ...
      <UModal
        ...
        title="Export XID Document"
        description="Export the XID document with privacy and signing options."
      >
        <template #content>
          <div class="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
            ...
            <div>
              ...
              <div class="space-y-1.5 bg-gray-50 dark:bg-gray-800/30 rounded-lg p-3">
                ...
                <label class="flex items-center gap-2 text-sm cursor-pointer">
                  <input
                    v-model="exportPrivateKeyOption"
                    type="radio"
                    value="Include"
                    name="pk-opt"
                  />
            ...
            <div v-if="exportPrivateKeyOption === 'Encrypt'">
              ...
              >
              <input
                v-model="exportPrivateKeyPassword"
                type="password"
                placeholder="Password for encrypting private keys"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              />
            </div>
            ...
            <div v-if="exportGeneratorOption === 'Encrypt' && generatorStatus === 'Included'">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Generator Password</label
              >
              <input
                v-model="exportGeneratorPassword"
                type="password"
                placeholder="Password for encrypting generator"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              />
            </div>
            ...
            <div
              v-if="
                exportPrivateKeyOption === 'Encrypt' ||
                (exportGeneratorOption === 'Encrypt' && generatorStatus === 'Included')
              "
            >
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Key Derivation Function</label
              >
              <select
                v-model.number="exportKdfMethod"
                class="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              >
            ...
            <div v-if="exportResult">
              ...
              <code
                class="block text-xs bg-gray-100 dark:bg-gray-800/50 p-3 rounded-lg font-mono break-all text-gray-600 dark:text-gray-400 max-h-32 overflow-y-auto"
                >{{ exportResult }}</code
              >
            </div>
          </div>
      ...
      <UModal
        ...
        title="Public Export"
        description="Your XID has been exported with private data elided."
      >
        <template #content>
          <div class="p-6 space-y-4">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Public Export</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400">
              Private keys and provenance generator have been elided. The digest tree is preserved
              for verification. The UR has been copied to your clipboard.
            </p>
      ...
      <UModal
        ...
        title="XID Templates"
        description="Choose a template to create a pre-configured XID document."
      >
        <template #content>
          <div class="p-6 space-y-4">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">XID Templates</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400">
              Choose a template to quickly create a pre-configured XID document. This will replace
