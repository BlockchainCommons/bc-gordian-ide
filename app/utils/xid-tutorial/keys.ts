import { Key, type Privilege, type XIDDocument } from "@blockchaincommons/xid";
import type { PublicKeys, Digest } from "@blockchaincommons/components";
...
export function setKeyPermissions(
  ...
  if (!key) return false;
  for (const p of key.allow) key.removeAllow(p);
  for (const p of allow) key.addAllow(p);
  doc.addKey(key);
  return true;
}
...
export function keyDisavowalInfo(doc: XIDDocument, nickname: string): DisavowedKey | undefined {
  ...
  return {
    ...
    assertionDigest: key.toEnvelope().digest(),
  };
}
