...
} from "@/utils/xid-tutorial/commitment";
  ...
  <div class="space-y-6">
    <section class="space-y-2">
      ...
      <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
        ...
        contract into an edge on your XID, or publish many digests in a single list so each blends
        into the crowd (<strong>herd privacy</strong>).
      </p>
    </section>
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border-2 border-indigo-200 dark:border-indigo-900 p-4 space-y-3">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          <UButton
            label="Compute digest"
            icon="i-heroicons-calculator"
            color="primary"
            @click="handleLoadContract"
          />
          <UButton
        ...
        <div
          v-if="contractEnvelope"
          class="space-y-2 pt-2 border-t border-gray-200 dark:border-gray-800"
        >
        ...
        <UAlert
          ...
          title="Edge attached"
          :description="`XID now has ${edgeList.length} edge(s).`"
        />
        <UButton
          v-if="edgeAttached"
      ...
      <div class="rounded-lg border-2 border-teal-200 dark:border-teal-900 p-4 space-y-3">
        <h3 class="text-sm font-semibold text-teal-700 dark:text-teal-300">
          Part B — Commitment list (herd privacy)
        </h3>
        ...
        <UButton
          v-if="uploadedClas.length"
          label="Generate README.md"
          icon="i-heroicons-document-plus"
          color="primary"
          @click="handleBuildList"
        />

        <UTextarea
          v-if="readmeMarkdown"
          :model-value="readmeMarkdown"
          readonly
          :rows="10"
          class="w-full font-mono text-xs"
        />
        ...
        >
          ...
          <UTextarea
            v-model="verifyPasteUr"
            placeholder="Paste a CLA UR to check"
            :rows="2"
            class="w-full font-mono text-xs"
          />
          <UButton
            label="Check against README"
            icon="i-heroicons-magnifying-glass"
            color="neutral"
            variant="outline"
            @click="handleVerifyAgainstList"
          />
          <UAlert
            v-if="verifyResult === true"
            color="success"
            title="✅ Digest found in commitment list"
          />
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-2">
        ...
        <div class="grid grid-cols-2 gap-x-6 text-xs">
          ...
          <div>1</div>
          <div>Trivial to identify</div>
          <div>10</div>
          <div>Difficult</div>
          <div>50</div>
          <div>Needle in haystack</div>
          <div>500</div>
          <div>Effectively anonymous</div>
        </div>
      </div>
