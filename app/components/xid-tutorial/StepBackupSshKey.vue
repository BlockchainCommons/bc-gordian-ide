...
import { CONFORMS_TO, VERIFIABLE_AT } from "@blockchaincommons/known-values";
...
function handleWrap() {
  ...
  // then metadata assertions. `subject type ur $SSH_PRVKEYS` == leaf(toCbor).
  const env = Envelope.leaf(ssh.prvKeys.toCbor())
...
function handleSplit() {
  ...
  const original = encryptedEnv.value.digest().toHex();
  ...
  const allMatch = combos.every(([a, b]) => {
    ...
    return res.ok && res.env.digest().toHex() === original;
  });
  ...
  <div class="space-y-6">
    <section class="space-y-2">
      ...
      <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
        Your XID isn't the only key worth protecting. Amira's SSH signing key (the one GitHub has on
        file, from §3.1) deserves the same treatment: wrap it in an annotated envelope,
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <div v-if="hasSshKey" class="space-y-1">
          ...
          <pre class="font-mono text-xs bg-gray-950 text-gray-200 p-2 rounded-md overflow-auto">{{
            sshKeyText
          }}</pre>
        </div>
        <UButton
          v-else
          label="Generate SSH signing key"
          icon="i-heroicons-key"
          color="primary"
          @click="handleGenerateSsh"
        />
      </div>
        ...
        >
          ...
          <div v-if="shares.length" class="space-y-2">
            <div v-for="(s, i) in shares" :key="i" class="space-y-1">
              ...
              <pre
                class="font-mono text-xs bg-gray-950 text-gray-200 p-2 rounded-md overflow-auto"
                >{{ shortShare(s) }}</pre>
            </div>
          </div>
          <UAlert
            v-if="recoveryResult"
            :color="recoveryResult.ok ? 'success' : 'error'"
