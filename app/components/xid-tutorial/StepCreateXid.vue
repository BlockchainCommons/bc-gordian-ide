...
const algorithmName = computed(() =>
  inceptionKey.value ? getKeyAlgorithm(inceptionKey.value) : "",
);
  ...
  <div class="space-y-6">
    <section class="space-y-2">
      ...
      <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
        A XID is a 32-byte identifier derived from the SHA-256 hash of your
        <em>inception public key</em>. Your private keys and provenance generator get encrypted with
        a password (Argon2id) and stored alongside the public material in a single signed document.
      </p>
    </section>
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between gap-2">
            ...
            <code class="text-xs font-mono text-gray-800 dark:text-gray-200 truncate flex-1">{{
              truncate(xidHex, 24)
            }}</code>
            <UButton
          ...
          <div class="flex items-center justify-between gap-2">
            ...
            <code class="text-xs font-mono text-gray-800 dark:text-gray-200 truncate flex-1">{{
              truncate(xidBytewords, 48)
            }}</code>
            <UButton
              :icon="
                copied === 'bytewords' ? 'i-heroicons-check' : 'i-heroicons-clipboard-document'
              "
        ...
        >
          ...
          <span v-if="password" class="text-xs text-gray-500"
            >· private key encrypted (Argon2id)</span
          >
        </div>
      </div>
      ...
      <div class="space-y-3">
        <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-200">
          Private (encrypted) vs. Public (elided)
        </h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div class="rounded-lg border-2 border-red-300 dark:border-red-800 p-3 space-y-2">
            ...
            <pre
              class="font-mono text-xs bg-gray-950 text-gray-200 dark:bg-gray-900 p-3 rounded-lg overflow-auto max-h-64"
              >{{ encryptedTree }}</pre>
          </div>
        ...
        <XidTutorialConceptCard
          title="Why does elision preserve signatures?"
          icon="i-heroicons-question-mark-circle"
        >
          Every element in an envelope has a cryptographic hash. Elision replaces the content with
          its hash but keeps the tree structure intact. Since signatures sign the root hash — not
          the content — signatures remain valid even when data is removed.
        </XidTutorialConceptCard>
      </div>
