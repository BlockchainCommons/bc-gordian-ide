...
const { activeSlot, activeDoc, activeIdentity, setActive, addSideKey, completeAndAdvance } =
  useXidTutorial();
  ...
  <div class="space-y-6">
    <section class="space-y-2">
      ...
      <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
        ...
        prove later that you held the claim at a specific time. The
        <em>commit-elide-reveal</em> pattern lets you sign a full attestation, publish
        <strong>only its digest</strong>, and reveal the full content to selected recipients later.
      </p>
    </section>
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <UButton
          ...
          color="primary"
          @click="handleBuildAndSign"
        />
      </div>

      <div
        v-if="fullSigned"
        class="rounded-lg border-2 border-red-200 dark:border-red-900 p-4 space-y-2"
      >
        <div class="flex items-center justify-between">
          <h3 class="text-sm font-semibold text-red-700 dark:text-red-300">
            Full signed attestation (keep private)
          </h3>
        ...
        <pre
          ...
          >{{ fullTree }}</pre>
        <UAlert
          v-if="fullSigValid === true"
          color="success"
          icon="i-heroicons-check-circle"
          title="Signature valid on full version"
        />
      </div>
      ...
      >
        <div class="flex items-center justify-between">
          <h3 class="text-sm font-semibold text-green-700 dark:text-green-300">
            Commitment (publish this)
          </h3>
          <UButton
            size="xs"
            variant="outline"
            label="Try to verify signature"
            @click="handleVerifyCommitment"
          />
        </div>
        <pre
          class="font-mono text-xs bg-gray-950 text-gray-200 p-3 rounded-md overflow-auto max-h-24"
          >{{ commitmentTree }}</pre>
        <UAlert
          v-if="digestMatch === true"
          color="success"
          icon="i-heroicons-check-circle"
      ...
      >
        <h3 class="text-sm font-semibold text-sky-800 dark:text-sky-200">
          What DevReviewer learns (when Amira reveals)
        </h3>
