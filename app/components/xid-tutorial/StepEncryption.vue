...
function ensureAttestationKey() {
  if (!activeSlot.value.attestationKey) {
    addSideKey("amira", "attestation", "Ed25519", ["Sign"]);
  }
}
...
function handleBuild() {
  ...
  if (!key) return;
  const xidUr = doc.xid.toUR().toString();
...
function handleWrongKey() {
  ...
  wrongDecryptStatus.value = res.ok
    ? "UNEXPECTED: decrypted with wrong key"
    : `Correctly rejected: ${res.reason}`;
}
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <form v-else autocomplete="off" class="space-y-3" @submit.prevent="ensureDevReviewer">
          ...
          <UButton
            type="submit"
            label="Create DevReviewer identity"
            color="primary"
      ...
      >
        ...
        <pre
          ...
          >{{ signedCleartextTree }}</pre>
        <UButton
          label="Encrypt for DevReviewer"
      ...
      <div
        v-if="encrypted"
        class="rounded-lg border-2 border-purple-200 dark:border-purple-900 p-4 space-y-2"
      >
        <h3 class="text-sm font-semibold text-purple-700 dark:text-purple-300">
          Encrypted (safe to send in the clear)
        </h3>
        <pre
          class="font-mono text-xs bg-gray-950 text-gray-200 p-3 rounded-md overflow-auto max-h-48"
          >{{ encryptedTree }}</pre>
        ...
        <div class="flex flex-wrap gap-2">
          <UButton
            label="Decrypt with DevReviewer's private key"
            icon="i-heroicons-lock-open"
            color="success"
            @click="handleDecrypt"
          />
          <UButton
            label="Try with a wrong key (Charlene)"
            icon="i-heroicons-shield-exclamation"
            color="warning"
            variant="outline"
            @click="handleWrongKey"
          />
        </div>
        <UAlert
          v-if="decryptStatus"
          :color="decryptStatus.startsWith('Decrypted') ? 'success' : 'error'"
          :title="decryptStatus"
        />
        <UAlert
          v-if="wrongDecryptStatus"
          :color="wrongDecryptStatus.startsWith('Correctly') ? 'success' : 'error'"
          :title="wrongDecryptStatus"
        />
      </div>
      ...
      >
        <h3 class="text-sm font-semibold text-green-700 dark:text-green-300">
          Decrypted by DevReviewer
        </h3>
        <pre
          class="font-mono text-xs bg-gray-950 text-gray-200 p-3 rounded-md overflow-auto max-h-48"
          >{{ decryptedTree }}</pre>
        <UAlert
          v-if="finalSigValid === true"
          color="success"
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-2">
        ...
        <div class="grid grid-cols-3 text-xs gap-2">
          ...
          <div>Public trace</div>
          <div>Yes — digest visible</div>
          <div>No — completely hidden</div>
          <div>Proves timing</div>
          <div>Yes</div>
          <div>No</div>
          <div>Who can see</div>
          <div>No one (until revealed)</div>
          <div>Specific recipients</div>
          <div>Use case</div>
          <div>"Might need to prove later"</div>
          <div>"Only this person should see"</div>
        </div>
      </div>
