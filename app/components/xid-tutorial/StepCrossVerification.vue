...
const githubResult = ref<{ keys: GithubKey[]; source: "live" | "canned"; error?: string } | null>(
  null,
);
...
const selectedEdge = computed<Envelope | null>(
  () => edgeList.value[selectedEdgeIndex.value]?.envelope ?? null,
);
  ...
  <div class="space-y-6">
    <section class="space-y-2">
      ...
      <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
        Self-consistency is necessary but not sufficient. To really trust a XID's claims, check them
        against <em>independent external sources</em>. Here: fetch GitHub's published signing keys
        and prove they match what's embedded in the edge — plus verify the edge's signature against
        the claimed key.
      </p>
    </section>
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <UButton
          ...
          color="neutral"
          variant="outline"
          @click="handleSelfConsistency"
        />
        <UAlert
          v-if="selfConsistent === true"
          color="success"
          icon="i-heroicons-check-circle"
          title="XID is self-consistent"
        />
        <UAlert
          v-else-if="selfConsistent === false"
          color="error"
          icon="i-heroicons-x-circle"
          title="Self-consistency failed"
        />
      </div>
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <UAlert
          ...
          color="warning"
          title="No edges on this XID"
          description="Go back to §3.1 and attach the GitHub edge."
        />
        <template v-else>
          <UFormField v-if="edgeList.length > 1" label="Which edge?">
            <USelect
              v-model="selectedEdgeIndex"
              :items="edgeList.map((_, i) => ({ label: `Edge ${i + 1}`, value: i }))"
              class="w-full"
            />
          </UFormField>
          <UButton
            label="Extract claimed SSH key + username"
          ...
          <div v-if="extracted" class="text-sm space-y-1 pt-2">
            <div>
              <span class="text-gray-500">Claimed username:</span>
              <code class="text-xs">{{ extracted.username ?? "(missing)" }}</code>
            </div>
            <div>
              <span class="text-gray-500">Claimed SSH key:</span>
              <code class="text-xs break-all">{{ extracted.claimedText ?? "(missing)" }}</code>
            </div>
            <div>
              <span class="text-gray-500">Profile URL:</span>
              <code class="text-xs break-all">{{ extracted.verifiableAt ?? "(missing)" }}</code>
            </div>
          </div>
        </template>
      </div>
      ...
      >
        ...
        <div class="flex items-center gap-2">
          ...
          <UCheckbox
            v-model="githubLive"
            label="Use live api.github.com (rate limited; CORS may block)"
          />
        </div>
        <UButton
          label="Fetch signing keys"
          icon="i-heroicons-cloud-arrow-down"
          color="primary"
          @click="handleFetchGithub"
        />
        <div v-if="githubResult" class="text-sm space-y-1">
          <UBadge :color="githubResult.source === 'live' ? 'success' : 'info'" variant="subtle">
            source: {{ githubResult.source }}
          </UBadge>
          <UAlert
            v-if="githubResult.error"
            color="warning"
            variant="subtle"
            :title="githubResult.error"
          />
          <template v-if="githubResult.keys[0]">
            <div>
              <span class="text-gray-500">GitHub key:</span>
              <code class="text-xs break-all">{{ githubResult.keys[0].key }}</code>
            </div>
            <div>
              <span class="text-gray-500">Registered:</span>
              <code class="text-xs">{{ githubResult.keys[0].created_at }}</code>
            </div>
          </template>
        </div>
        <UAlert
      ...
      >
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          Since the XID's claimed key matches GitHub, verifying the edge's signature with that key
          proves BRadvoc8 controls the matching SSH private key — i.e. proof of control.
        </p>
        <UButton
          label="Verify edge signature"
          icon="i-heroicons-shield-check"
          color="primary"
          @click="handleVerifyEdgeSig"
        />
        <UAlert
          v-if="edgeSignatureValid === true"
          color="success"
          icon="i-heroicons-check-circle"
        ...
        />
        <UAlert
          v-else-if="edgeSignatureValid === false"
          color="error"
          icon="i-heroicons-x-circle"
      ...
      <div
        v-if="canContinue"
        class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-2 bg-green-50/30 dark:bg-green-950/10"
      >
        ...
        <ul class="text-sm text-gray-700 dark:text-gray-300 space-y-1">
          ...
          <li v-if="githubResult?.keys[0]?.created_at">
            🗓️ Key in use since {{ githubResult.keys[0].created_at }} (temporal anchor)
          </li>
        </ul>
      </div>
