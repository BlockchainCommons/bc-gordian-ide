...
const permToDrop = ref<Privilege | undefined>("Access");
...
const selectedEntry = computed(
  () => keyInventory.value.find((k) => k.nickname === selectedNickname.value) ?? null,
);
...
function handleDrop() {
  ...
  const pub = decodeURWith(UR.parse(entry.pubKeysUr), PublicKeys.codec);
...
function handleRotate() {
  ...
  if (side) {
    rotatedUr.value = side.pubKeys.toUR().toString();
...
<template>
  <div class="space-y-6">
    ...
    />
    <UAlert
      v-else-if="operationalNicknames.length === 0"
      color="warning"
      icon="i-heroicons-exclamation-triangle"
      title="No operational keys"
      description="Add a laptop/portable key in §5.1 first."
    />
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <div v-if="findResult" class="text-sm">
          <template v-if="findResult.found">
            ...
            <UBadge
              v-for="p in findResult.permissions"
              :key="String(p)"
              size="xs"
              color="primary"
              variant="soft"
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <div v-if="dropDone" class="text-sm">
          ...
          <div class="mt-1">
            ...
            <UBadge
              v-for="p in selectedEntry?.permissions ?? []"
              :key="String(p)"
              size="xs"
              color="primary"
              variant="soft"
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <div v-if="rotateDone" class="space-y-1">
          <UAlert
            ...
            variant="subtle"
            icon="i-heroicons-check-circle"
            title="Key rotated — old key removed"
          />
          <pre
