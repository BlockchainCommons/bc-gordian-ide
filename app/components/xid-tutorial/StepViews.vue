...
function handleGenerateView() {
  ...
  for (const i of selectedEdgeIndices.value) {
    ...
    if (!edge) continue;
    try {
      env = env.elide({ removing: [edge.envelope] });
    } catch {
      /* already elided */
    }
  }
  ...
  for (const i of selectedKeyIndices.value) {
    ...
    if (!k) continue;
    try {
      env = env.elide({ removing: [k.toEnvelope({ privateKeys: "elide" })] });
    } catch {
      /* */
    }
  }
...
const baseDigest = computed(() => {
  ...
  if (!doc) return "";
  try {
    return exportPublicEnvelope(doc).digest().toHex();
  } catch {
    return "";
  }
});
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <div>
          ...
          <div class="space-y-1">
            <label v-for="(k, i) in keyList" :key="i" class="flex items-center gap-2 text-sm">
              <UCheckbox
        ...
        <div>
          ...
          <div v-if="edgeList.length === 0" class="text-xs text-gray-400 italic">
            No edges on this XID yet
          </div>
          <div v-else class="space-y-1">
            <label v-for="(e, i) in edgeList" :key="i" class="flex items-center gap-2 text-sm">
              <UCheckbox
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <UBadge v-if="v.digestHex === baseDigest" color="success" size="xs" variant="subtle">
                same root digest
              </UBadge>
              <UBadge v-else color="warning" size="xs" variant="subtle"> different digest </UBadge>
            </div>
            <UBadge v-if="v.sigValid" color="success" size="xs" variant="subtle"
              >✅ sig valid</UBadge
            >
          </div>
          <pre
