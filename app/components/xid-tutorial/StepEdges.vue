...
function handleBuildEdge() {
  ...
  const sshPubUr = ssh.pubKeys.toUR().toString();
  ...
  <div class="space-y-6">
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <div v-if="hasSshKey" class="space-y-2">
          <div class="flex items-center gap-2 text-sm">
            ...
            <span
              >Standalone SSH signing key generated. Its public counterpart will go inside the edge
              target; the private half signs the edge.</span
            >
          </div>
          <pre class="font-mono text-xs bg-gray-950 text-gray-200 p-3 rounded-md overflow-auto">{{
            sshKeyText
          }}</pre>
      ...
      >
        ...
        <pre
          class="font-mono text-xs bg-gray-950 text-gray-200 p-3 rounded-md overflow-auto max-h-96"
          >{{ builtEdgeTree }}</pre>
        <p class="text-xs text-gray-500">
      ...
      <div
        v-if="edgeAttached"
        class="rounded-lg border border-green-200 dark:border-green-900 p-4 space-y-2"
      >
        <div class="flex items-center gap-2">
          <UIcon name="i-heroicons-check-circle" class="w-5 h-5 text-green-500" />
          <span class="text-sm"
            >Edge attached. Your XID now has {{ edgeList.length }} edge(s).</span
          >
        </div>
        <UButton
