      ...
      <template v-else>
        ...
        <div v-if="notation || hex || diagnostic">
          <div class="flex items-center justify-between mb-2">
            ...
            >
              {{ envelopeLabel }}
              <UBadge
                v-if="envelopeOutputMode === 'hex' && hex"
                color="neutral"
                variant="soft"
                size="xs"
                class="ml-1"
                >{{ hex.length / 2 }} bytes</UBadge
              >
            </h4>
          ...
          <pre
            v-if="envelopeOutputMode === 'notation' && notation"
            class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-800 dark:text-gray-200 overflow-x-auto whitespace-pre max-h-48"
            >{{ notation }}</pre>
          <div
            v-else-if="envelopeOutputMode === 'hex' && hex"
          ...
          <pre
            v-else-if="envelopeOutputMode === 'diag' && diagnostic"
            class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-800 dark:text-gray-200 overflow-x-auto whitespace-pre max-h-48"
            >{{ diagnostic }}</pre>
        </div>
        ...
        <div v-if="tree">
          <div class="flex items-center justify-between mb-2">
            ...
            <UButton
              :icon="
                copiedLabel === 'tree' ? 'i-heroicons-check' : 'i-heroicons-clipboard-document'
              "
          ...
          <pre
            class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-800 dark:text-gray-200 overflow-x-auto whitespace-pre max-h-64"
            >{{ tree }}</pre>
        </div>
        ...
        <div v-if="ur">
          ...
          <div
            class="bg-gray-100 dark:bg-gray-800/50 rounded-lg p-3 text-xs font-mono text-gray-600 dark:text-gray-400 break-all max-h-32 overflow-y-auto"
          >
            {{ ur }}
          </div>
        </div>
