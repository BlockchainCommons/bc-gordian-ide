...
function ensureContractKey() {
  if (!identities.value.amira.contractKey) {
    addSideKey("amira", "contract", "Ed25519", ["Sign"]);
  }
}
...
function handleBuild() {
  ...
  const env = buildClaEnvelope({
    ...
    projectManagerXidUr: ben.xid.toUR().toString(),
    projectManagerNickname: "Ben (SisterSpaces)",
    contributorXidUr: amira.xid.toUR().toString(),
...
function handleAccept() {
  ...
  const accepted = acceptCla(signedCla.value, ben.xid.toUR().toString(), benKey, new Date());
  ...
  setArtifact("acceptedCla", accepted.toUR().toString());
}
...
function handleVerifyContribSig() {
  ...
  try {
    contribSigValid.value = signedCla.value.hasSignatureFrom(contractPub);
  } catch {
    contribSigValid.value = false;
  }
}
...
function handleVerifyAcceptSig() {
  ...
  try {
    accepterSigValid.value = acceptedCla.value.hasSignatureFrom(benPub);
  } catch {
    accepterSigValid.value = false;
  }
}
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <UFormField label="License name"
          ><UInput v-model="licenseName" class="w-full"
        /></UFormField>
        ...
        <UFormField label="Copyright terms"
          ><UInput v-model="copyrightTerms" class="w-full"
        /></UFormField>
        <UFormField label="Patent terms"
          ><UInput v-model="patentTerms" class="w-full"
        /></UFormField>
        <UFormField label="Contributor represents"
          ><UInput v-model="contributorRepresents" class="w-full"
        /></UFormField>
        <UButton
          label="Build CLA envelope"
      ...
      <div
        v-if="claEnvelope"
        class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-2"
      >
        ...
        <pre
          ...
          >{{ claTree }}</pre>
        <UButton
          label="Amira signs with contract key"
      ...
      <div
        v-if="signedCla"
        class="rounded-lg border-2 border-blue-200 dark:border-blue-900 p-4 space-y-2"
      >
        <div class="flex items-center justify-between">
          ...
          <UButton
            size="xs"
            variant="outline"
            label="Verify contributor signature"
            @click="handleVerifyContribSig"
          />
        </div>
        <pre
          class="font-mono text-xs bg-gray-950 text-gray-200 p-3 rounded-md overflow-auto max-h-48"
          >{{ signedClaTree }}</pre>
        <UAlert
          v-if="contribSigValid === true"
          color="success"
          icon="i-heroicons-check-circle"
          title="Contributor signature valid"
        />
        <UAlert
          v-else-if="contribSigValid === false"
          color="error"
          icon="i-heroicons-x-circle"
          title="Contributor signature failed"
        />
        <UButton
          label="Ben accepts + countersigns"
      ...
      <div
        v-if="acceptedCla"
        class="rounded-lg border-2 border-green-200 dark:border-green-900 p-4 space-y-2"
      >
        <div class="flex items-center justify-between">
          <h3 class="text-sm font-semibold text-green-700 dark:text-green-300">
            Accepted CLA (double-wrapped)
          </h3>
          <UButton
            size="xs"
            variant="outline"
            label="Verify acceptance signature"
            @click="handleVerifyAcceptSig"
          />
        </div>
        <pre
          class="font-mono text-xs bg-gray-950 text-gray-200 p-3 rounded-md overflow-auto max-h-64"
          >{{ acceptedClaTree }}</pre>
        <UAlert
          v-if="accepterSigValid === true"
          color="success"
          icon="i-heroicons-check-circle"
          title="Accepter signature valid"
        />
        <UButton
          label="Advance Amira's provenance"
