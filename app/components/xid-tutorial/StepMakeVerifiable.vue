...
import { DEREFERENCE_VIA } from "@blockchaincommons/known-values";
...
function runTamper() {
  ...
  if (!env) return;
  tamperedUr.value = tamperMiddle(env.toUR().toString());
  try {
    const parsed = decodeURWith(UR.parse(tamperedUr.value), Envelope.codec);
    XIDDocument.fromEnvelope(parsed, { verify: "inception" });
...
function runBenChecks() {
  ...
  let parsed: Envelope;
  try {
    parsed = decodeURWith(UR.parse(benPastedUr.value.trim()), Envelope.codec);
  } catch (e) {
  ...
  try {
    const inner = parsed.subject().isWrapped() ? parsed.unwrap() : parsed;
    ...
    for (const a of assertions) {
      ...
      try {
        ...
        if (predKv && predKv.equals(DEREFERENCE_VIA)) {
          ...
          try {
            const uri = c.assertion.object().expectSubject((cbor) => URI.fromCbor(cbor));
  ...
  if (derefUris.length === 0) {
    ...
    add(
      "dereferenceVia match",
      "pass",
      `Fetched URL matches one of ${derefUris.length} dereferenceVia URIs.`,
    );
  } else if (benFetchUrl.value) {
    add(
      "dereferenceVia match",
      "warn",
      `Fetched URL does not match. Declared URIs: ${derefUris.join(", ")}.`,
    );
  } else {
    add(
      "dereferenceVia match",
      "warn",
      `No fetch URL supplied. XID declares: ${derefUris.join(", ")}.`,
    );
  }

  try {
    const doc = XIDDocument.fromEnvelope(parsed, { verify: "none" });
    const mark = doc.provenance;
    if (mark) {
      const cid = Array.from(mark.chainId.slice(0, 4))
    ...
    add(
      ...
      "fail",
      e instanceof Error ? e.message : "Could not read provenance",
    );
  }
}
...
function useCurrentPublic() {
  const env = getPublicEnvelope();
  if (env) benPastedUr.value = env.toUR().toString();
    ...
    <section class="space-y-2">
      <div class="flex items-center gap-3">
        ...
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">
          Make Your XID Verifiable
        </h2>
      </div>
      <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
        Right now anyone could pass around a stale copy of your XID and you'd have no way to flag
        it. Declaring a
        <code class="text-xs bg-gray-100 dark:bg-gray-800 px-1 rounded">dereferenceVia</code>
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <template v-if="provenanceMark">
          ...
          <UCheckbox
            v-model="hasPublished"
            label="I have published this edition — advance the provenance mark for the next one"
          />
          <UButton
          ...
          <p class="text-xs text-gray-500">
            Only advance once you've actually published this edition — the chain orders
            <em>published</em> editions.
          </p>
        </template>
        <UAlert
          v-else
      ...
      <!-- Tampering demo -->
      <div
        class="rounded-lg border border-amber-300 dark:border-amber-800 p-4 space-y-3 bg-amber-50/50 dark:bg-amber-950/10"
      >
        <div class="flex items-center gap-2">
      ...
      <!-- Ben's perspective -->
      <div
        class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3 bg-sky-50/30 dark:bg-sky-950/10"
      >
        <div class="flex items-center gap-2">
          ...
          <h3 class="text-sm font-semibold text-sky-800 dark:text-sky-200">
            Ben's perspective — verify a received XID
          </h3>
        </div>
        <p class="text-sm text-sky-800 dark:text-sky-200">
          Ben got a XID via email. He fetches the canonical version from the declared URL and runs
          three checks: inception signature, dereferenceVia URL match, and provenance.
        </p>
        <UButton
          label="Use current public XID & first URL"
          variant="soft"
          color="info"
          @click="useCurrentPublic"
        />
