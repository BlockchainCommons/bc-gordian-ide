...
async function handleSplit() {
  ...
  await nextTick();
  try {
    originalDigest.value = env.digest().toHex();
...
function handleRecover() {
  const provided = [recover1.value, recover2.value, recover3.value]
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  const match = res.env.digest().toHex() === originalDigest.value;
  ...
  <div class="space-y-6">
    ...
    <template v-if="activeDoc">
      ...
      >
        ...
        /></UFormField>
        <UFormField label="Share C (optional)"
          ><UTextarea v-model="recover3" :rows="2" class="w-full font-mono text-xs"
        /></UFormField>
        <UButton
          label="Reconstruct & verify"
