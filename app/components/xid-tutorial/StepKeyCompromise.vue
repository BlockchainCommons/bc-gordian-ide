...
const operationalNicknames = computed(() =>
  keyInventory.value
    .filter((k) => !k.isInception)
    .map((k) => ({ label: k.nickname, value: k.nickname })),
);
...
function handleReconstruct() {
  ...
  try {
    const doc = loadXidFromUr(res.env.toUR().toString(), activeSlot.value.password || undefined);
    ...
    reconstructResult.value = {
      ok: false,
      message: e instanceof Error ? e.message : "Could not load recovered XID",
    };
  }
}
...
function handleBuildDisavowal() {
  ...
  const signed = buildSignedDisavowal(
    {
      disavowerXidUr: doc.xid.toUR().toString(),
  ...
  <div class="space-y-6">
    <section class="space-y-2">
      ...
      <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
        The worst case: an operational key is compromised. Because Amira kept her master key offline
        and SSKR-backed (§5.3), she can recover her full authority, revoke the bad keys, rotate in
        replacements, advance provenance, and rebuild a clean operational XID — all while keeping
        the same identifier.
      </p>
    </section>
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        />
        <UFormField label="Share A"
          ><UTextarea v-model="recover1" :rows="2" class="w-full font-mono text-xs"
        /></UFormField>
        <UFormField label="Share B"
          ><UTextarea v-model="recover2" :rows="2" class="w-full font-mono text-xs"
        /></UFormField>
        <UButton
          label="Reconstruct master XID"
          icon="i-heroicons-puzzle-piece"
          color="primary"
          @click="handleReconstruct"
        />
        <UAlert
          v-if="reconstructResult"
          :color="reconstructResult.ok ? 'success' : 'error'"
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <div class="flex flex-wrap gap-2 items-end">
          ...
          <UButton
            ...
            color="error"
            :disabled="!revokeNickname || revokeDone"
            @click="handleRevoke"
          />
        </div>
        <UAlert
          ...
          title="Compromised key removed"
        />
        <template v-if="revokeDone">
          <UButton
            label="Add replacement key (…-may2026)"
          ...
          <UAlert
            ...
            variant="subtle"
            icon="i-heroicons-check-circle"
            title="Replacement key added"
          />
        </template>
        <template v-if="rekeyDone">
          <UButton
            label="Advance provenance & re-publish"
            icon="i-heroicons-arrow-path"
            variant="outline"
            color="neutral"
            :disabled="provenanceAdvanced"
            @click="handleAdvance"
          />
          <UAlert
            v-if="provenanceAdvanced"
            color="success"
            variant="subtle"
            icon="i-heroicons-check-circle"
            title="Provenance advanced"
          />
        </template>
      </div>
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-2">
        ...
        >
          <UIcon
            :name="k.isInception ? 'i-heroicons-star' : 'i-heroicons-key'"
            class="w-4 h-4 shrink-0"
            :class="k.isInception ? 'text-amber-500' : 'text-gray-400'"
          />
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <UButton
          label="Rebuild operational XID"
          icon="i-heroicons-cube"
          color="primary"
          @click="handleRebuild"
        />
        <pre
          v-if="operationalBuilt"
          class="font-mono text-xs bg-gray-950 text-gray-200 p-3 rounded-md overflow-auto max-h-72"
          >{{ operationalTree }}</pre>
      </div>

      <!-- Step D: disavowal statement -->
      <div
        v-if="rekeyDone && disavowedKeys.length > 0"
      ...
      >
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          ...
          publishes a signed statement — structured as a standard edge (§3.1) and signed by her new
          key — naming the compromised keys so verifiers can distrust signatures from the compromise
          window. She can publish it alongside her XID without adding it as an edge.
        </p>
        <UButton
          label="Build &amp; sign disavowal"
          icon="i-heroicons-megaphone"
          color="primary"
          @click="handleBuildDisavowal"
        />
        <div v-if="disavowalBuilt" class="space-y-2">
          <UAlert
            ...
            :title="
              disavowalVerified
                ? 'Disavowal signed & verified'
                : 'Disavowal signature did not verify'
            "
      ...
      <div
        v-if="canFinish"
        class="rounded-lg border border-green-200 dark:border-green-900 p-4 bg-green-50/30 dark:bg-green-950/10"
      >
