...
const { activeDoc, edgeList, advanceProvenance, removeEdgeFromActive, completeAndAdvance } =
  useXidTutorial();
...
function parseUrl(ur: string): Envelope | null {
  try {
    return decodeURWith(UR.parse(ur.trim()), Envelope.codec);
  } catch {
    return null;
  }
}
...
function handleCompare() {
  ...
  if (!env1 || !env2) return;
  try {
    const docs = [env1, env2].map((e) => XIDDocument.fromEnvelope(e, { verify: "none" }));
    const marks = docs
      .map((d) => d.provenance)
    ...
    if (marks.length !== 2) {
      comparisonResult.value = {
        ...
        sameChain: false,
        latestIndex: -1,
        issues: ["One or both XIDs have no provenance mark"],
      };
      return;
    }
    ...
    comparisonResult.value = {
      ...
      sameChain: false,
      latestIndex: -1,
      issues: [e instanceof Error ? e.message : "Failed"],
    };
  }
}
...
function useCurrent(slot: "paste1" | "paste2") {
  ...
  if (!doc) return;
  try {
    const ur = exportPublicEnvelope(doc).toUR().toString();
  ...
  <div class="space-y-6">
    <section class="space-y-2">
      ...
      <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
        Views are reversible; editions are not. Removing content entirely creates a new edition with
        implicit revocation — anything absent in the newer edition is revoked. Provenance marks tell
        verifiers which edition is newest and whether they're part of the same chain.
      </p>
    </section>
    ...
    <template v-if="activeDoc">
        ...
        <template v-else>
          <UFormField label="Edge to remove">
            <USelect
              ...
              :items="
                edgeList.map((e, i) => ({
                  label: `Edge ${i + 1} · ${e.digestHex.slice(0, 12)}…`,
                  value: i,
                }))
              "
              class="w-full"
      ...
      <div class="rounded-lg border-2 border-sky-200 dark:border-sky-900 p-4 space-y-3">
        <h3 class="text-sm font-semibold text-sky-700 dark:text-sky-300">
          Compare editions (SecurityMaintainer scenario)
        </h3>
        <p class="text-sm text-sky-800 dark:text-sky-200">
          Given two copies of a XID, use provenance marks to pick the newest and detect whether both
        ...
        <UButton
          size="xs"
          variant="outline"
          label="Use current XID"
          @click="useCurrent('paste1')"
        />
