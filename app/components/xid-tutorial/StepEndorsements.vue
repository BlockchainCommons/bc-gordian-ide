...
import type { Envelope } from "@blockchaincommons/envelope/all";
import {
...
const reviewerPeer = ref(
  "Writes secure, well-tested code with clear attention to privacy-preserving patterns",
);
const reviewerContext = ref(
  "Verified previous security experience, worked together on short project for SisterSpaces",
);
const reviewerScope = ref("Security architecture, cryptographic implementation, privacy patterns");
const reviewerBasis = ref(
  "Security collaboration partner who verified credentials through commit-reveal and encrypted sharing",
);
  ...
  const signed = buildSignedAttestation(
    {
      ...
      sourceXidUr: charleneDoc.xid.toUR().toString(),
      targetXidUr: amiraDoc.xid.toUR().toString(),
      date: new Date(),
      extras: {
...
function handleVerifyDetached() {
  ...
  detachedVerified.value = verifyAttestationAgainstXid(
    detachedEndorsement.value,
    charleneDoc,
  ).verified;
}
  ...
  const edge = buildSignedEdge(
    {
      ...
      isA: "attestation",
      source: {
        xidUr: reviewerDoc.xid.toUR().toString(),
        extras: {
      ...
      target: {
        xidUr: amiraDoc.xid.toUR().toString(),
        extras: {
...
function handleForgery() {
  ...
  const amiraKey =
    identities.value.amira.attestationKey?.prvKeys ??
    identities.value.amira.document?.inceptionKey?.privateKeys;
  const charleneDoc = identities.value.charlene.document;
  const amiraDoc = identities.value.amira.document;
  ...
  const fake = buildSignedAttestation(
    {
      ...
      sourceXidUr: charleneDoc.xid.toUR().toString(), // lies about source
      targetXidUr: amiraDoc.xid.toUR().toString(),
      date: new Date(),
    },
  ...
  <div class="space-y-6">
    <section class="space-y-2">
      ...
      <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
        Self-attestations are signed <em>by you</em> — limited weight. Peer endorsements are signed
        <em>by others</em>, who stake their own reputation on you. Multiple endorsements from
        different contexts triangulate trust. Two forms: <strong>detached</strong> (a separate
        signed envelope) and <strong>embedded</strong> (an edge inside the subject's XID).
      </p>
    </section>
    ...
    <UAlert
      ...
      title="Need a XID first"
      description="Go back to §1.3."
    />
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border-2 border-emerald-200 dark:border-emerald-900 p-4 space-y-3">
        <div class="flex items-center gap-2">
          ...
          <h3 class="text-sm font-semibold text-emerald-700 dark:text-emerald-300">
            Part 1 — Detached endorsement (Charlene)
          </h3>
        </div>
        <form
          v-if="!identities.charlene.document"
          autocomplete="off"
          class="space-y-3"
          @submit.prevent="ensureCharlene"
        >
          <input
            type="text"
            name="username"
        ...
        <template v-else>
          <div class="text-xs text-gray-500">
            Charlene's XID:
            <code>{{ identities.charlene.document.xid.toUR().toString().slice(0, 40) }}…</code>
          </div>
          ...
          <div class="flex gap-2">
            <UButton
              label="Sign as Charlene"
              icon="i-heroicons-pencil-square"
              color="primary"
              @click="handleBuildDetached"
            />
            <UButton
              v-if="detachedEndorsement"
              label="Verify against Charlene's keys"
              icon="i-heroicons-check-badge"
              variant="outline"
              @click="handleVerifyDetached"
            />
          </div>
        </template>
        <pre
          v-if="detachedTree"
          class="font-mono text-xs bg-gray-950 text-gray-200 p-3 rounded-md overflow-auto max-h-64"
          >{{ detachedTree }}</pre>
        <UAlert
          v-if="detachedVerified === true"
          color="success"
          icon="i-heroicons-check-circle"
          title="Verified — signed by Charlene's inception key"
        />
        <UAlert
          v-else-if="detachedVerified === false"
          color="error"
      ...
      <div class="rounded-lg border-2 border-violet-200 dark:border-violet-900 p-4 space-y-3">
        <div class="flex items-center gap-2">
          ...
          <h3 class="text-sm font-semibold text-violet-700 dark:text-violet-300">
            Part 2 — Embedded edge (DevReviewer)
          </h3>
        </div>
        <form
          v-if="!identities.devreviewer.document"
          autocomplete="off"
          class="space-y-3"
          @submit.prevent="ensureDevReviewer"
        >
          <input
            type="text"
            name="username"
          ...
          <UButton
            type="submit"
            label="Create DevReviewer's XID"
            color="primary"
            icon="i-heroicons-user-plus"
          />
        </form>
        <template v-else>
          <div class="text-xs text-gray-500">
            DevReviewer's XID:
            <code>{{ identities.devreviewer.document.xid.toUR().toString().slice(0, 40) }}…</code>
          </div>
          ...
          <div class="flex gap-2">
            <UButton
              label="Build + sign edge"
              icon="i-heroicons-link"
              color="primary"
              @click="handleBuildEmbedded"
            />
            <UButton
              v-if="embeddedEdge"
      ...
      <!-- Forgery demo -->
      <div
        class="rounded-lg border border-amber-300 dark:border-amber-800 p-4 space-y-2 bg-amber-50/50 dark:bg-amber-950/10"
      >
        <h3 class="text-sm font-semibold text-amber-800 dark:text-amber-200">Forgery demo</h3>
        <p class="text-sm text-amber-800 dark:text-amber-200">
          An attacker can't produce a valid endorsement without the endorser's private key — watch
          what happens when we try.
        </p>
        <UButton
          label="Try to forge Charlene's endorsement"
        ...
        <UAlert
          v-if="forgeryStatus"
          :color="forgeryStatus.startsWith('✅') ? 'success' : 'error'"
          :title="forgeryStatus"
        />
      </div>
