...
import type { Privilege } from "@blockchaincommons/xid";
...
const OPERATIONAL_PRIVILEGES: { value: Privilege; label: string }[] = [
  ...
  { value: "Elide", label: "elide" },
  { value: "Issue", label: "issue" },
  { value: "Access", label: "access" },
];

const LAPTOP_PRESET: Privilege[] = ["Auth", "Sign", "Encrypt", "Elide", "Issue", "Access"];
const PORTABLE_PRESET: Privilege[] = ["Auth", "Sign", "Elide", "Access"];
...
function handleAddKey() {
  ...
  if (side) {
    lastAddedUr.value = side.pubKeys.toUR().toString();
    addedCount.value++;
  }
}
  ...
  <div class="space-y-6">
    <section class="space-y-2">
      ...
      <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
        ...
        <a
          href="https://www.blockchaincommons.com/musings/Least-Necessary/"
          target="_blank"
    ...
    <template v-if="activeDoc">
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <div class="space-y-2">
          ...
          >
            ...
            <UBadge v-if="k.isInception" size="xs" color="warning" variant="subtle"
              >inception</UBadge
            >
      ...
      <div class="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          The operational XID is an elided-private copy with the inception (master) key removed, so
          a compromised laptop can't rotate keys or revoke the identity. Your full master stays
          intact here — you'll back it up in §5.3 and use it to recover in §5.5.
        </p>
        <UButton
          label="Build operational XID"
          icon="i-heroicons-cube"
          color="primary"
          @click="handleBuildOperational"
        />
