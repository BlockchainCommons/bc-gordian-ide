  ...
  switch (node.type) {
    case "subject":
      ...
      if (node.valueType === "known") {
        return knownValueNames[String(node.value)]
          ? `'${knownValueNames[String(node.value)]}'`
          : `known(${node.value})`;
      }
    ...
    case "assertion": {
      const predicateLabel =
        node.predicateType === "known" && node.predicateKnownValue
    ...
    case "signed": {
      ...
      if (node.signerIds && node.signerIds.length > 1)
        parts.push(`(${node.signerIds.length} signers)`);
    ...
    case "encrypted": {
      ...
      if (node.encryptionMode === "recipient" && node.recipientCount)
        return `ENCRYPTED (${node.recipientCount} recipients)`;
...
function getNodeTypeLabel(type: NodeType): string {
  ...
  switch (type) {
    ...
    case "signed":
      return n.signingAlgorithm ? `Signed (${n.signingAlgorithm})` : "Signed Envelope";
    case "encrypted": {
    ...
    case "compressed":
      return n.compressSubjectOnly ? "Subject Compressed" : "Compressed Envelope";
    case "elided":
  ...
  <div class="tree-node">
    ...
    >
      ...
      <div class="flex-1 flex items-center gap-2 min-w-0">
        ...
        <template
          v-else-if="['wrapped', 'signed', 'encrypted', 'compressed', 'elided'].includes(node.type)"
        >
          <span :class="['text-sm font-medium', getNodeColor(node.type)]">{{
            getNodeDisplay(node)
          }}</span>
            ...
            :color="
              node.valueType === 'string'
                ...
                : node.valueType === 'number' || node.valueType === 'bigint'
                  ...
                  : node.valueType === 'date'
                    ? 'info'
                    : node.valueType === 'uuid' || node.valueType === 'arid'
                      ? 'success'
                      : node.valueType === 'uri'
                        ? 'info'
                        : node.valueType === 'known'
                          ? 'error'
                          : 'neutral'
            "
            variant="soft"
    ...
    <template v-if="node.expanded">
      ...
      <EnvelopeTreeNode
        ...
        @reorder-assertions="
          emit('reorderAssertions', $event[0] as string, $event[1] as unknown as EnvelopeNode[])
        "
      />
      ...
      >
        <div
          v-for="assertion in localAssertions"
          :key="assertion.id"
          class="assertion-item relative group/drag"
        >
          <!-- Drag handle -->
          <div
